...
"use strict";var timeout=1e3*(0<context.timeout?context.timeout+1:30);function initControlToggle(renderedFeebacks,$itemContainer,$controls,$toggleContainer,toggleButtonTemplate,callback){var $ok=$(toggleButtonTemplate()).click(function(){_.each(renderedFeebacks,function(fb){fb.dom.hide()}),function(interactionContainers){_.each(interactionContainers,function($interaction){$interaction.find(".interaction-cover").remove()})}([$itemContainer]),$ok.remove(),$controls.show(),callback()});$controls.hide(),$toggleContainer.append($ok),function(interactionContainers){var $cover=$('<div class="interaction-cover modal-bg">');_.each(interactionContainers,function(
$interaction){$interaction.append($cover)})}([$itemContainer])}return{showFeedbacks:function(item,loader,renderer,itemSession,onCloseCallback,onShowCallback){var firstFeedback,interactionsDisplayInfo=function(item){var interactionsDisplayInfo=[],$itemContainer=item.getContainer(),interactionOrder=0;return _.each(item.getComposingElements(),function(element){element.is("interaction")&&(element.attr("responseIdentifier"),interactionsDisplayInfo.push(function(interaction){var messageGroupId,$displayContainer,$interactionContainer=interaction.getContainer(),responseIdentifier=interaction.attr("responseIdentifier");return interaction.is("inlineInteraction")?(
$displayContainer=$interactionContainer.closest('[class*=" col-"], [class^="col-"]'),(messageGroupId=$displayContainer.attr("data-messageGroupId"))||(messageGroupId=_.uniqueId("inline_message_group_"),$displayContainer.attr("data-messageGroupId",messageGroupId))):(messageGroupId=responseIdentifier,$displayContainer=$interactionContainer),{responseIdentifier:responseIdentifier,interactionContainer:$interactionContainer,displayContainer:$displayContainer,messageGroupId:messageGroupId,order:-1}}(element)))}),$itemContainer.find(".qti-interaction").each(function(){var interactionContainer=this;_.each(interactionsDisplayInfo,function(_interactionInfo){if(
...
start:function(){var runnerContext=window.tao.qtiRunnerContext;qtiBootstrap(runnerContext)}}}),define("taoQtiItemloaderqtiLoader.bundle",function(){}),window.bundles=(window.bundles||[]).concat(["taoQtiItem/loader/taoQtiItem.min"]);
