...
min<=1?"You must at least %d choice":"You must select at least %d choices",min),this.appendInstruction(interaction,msg,function(){getResponse(interaction).length>=min?this.setLevel("success"):this.reset()}))},appendNotification:function(element,message,level){if(level=level||"info",Instruction.isValidLevel(level)){var $container=containerHelper.get(element);$container.find(".notification-container").prepend(notifTpl({level:level,message:message}));var $notif=$container.find(".item-notification:first"),_remove=function(){$notif.fadeOut()};return $notif.find(".close-trigger").on("click",_remove),setTimeout(_remove,2e3),$notif}},removeNotifications:function(
element){containerHelper.get(element).find(".item-notification").remove()}}}),define("taoQtiItem/qtiCommonRenderer/helpers/itemStylesheetHandler",["jquery","lodash"],function($,_){"use strict";var informLoaded=_.throttle(function(){$(document).trigger("customcssloaded.styleeditor")},10,{leading:!1});return{attach:function(stylesheets){var $head=$("head");$head.length||($head=$("body")),_(stylesheets).forEach(function(stylesheet){var $link,href;stylesheet.attr("href")&&(href=($link=$(stylesheet.render())).attr("href"),$link.removeAttr("href").attr("href",href),$link.one("load",informLoaded).appendTo($head))})},detach:function(stylesheets){_(stylesheets
).forEach(function(stylesheet){stylesheet.serial&&$('link[data-serial="'+stylesheet.serial+'"]').remove()})}}}),define("taoQtiItem/qtiCommonRenderer/helpers/patternMask",[],function(){"use strict";var regexChar=/\^\[\\s\\S\]\{\d+\,(\d+)\}\$/,regexWords=/\^\(\?\:\(\?\:\[\^\\s\\:\\!\\\?\\\;\\\…\\\€\]\+\)\[\\s\\:\\!\\\?\\;\\\…\\\€\]\*\)\{\d+\,(\d+)\}\$/;return{parsePattern:function(pattern,type){return null==pattern?null:"words"!==type?"chars"!==type?null:null!==(result=pattern.match(regexChar))&&1<result.length?result[1]:null:null!==(result=pattern.match(regexWords))&&1<result.length?result[1]:null;var result},createMaxWordPattern:function(max){
return"^(?:(?:[^\\s\\:\\!\\?\\;\\…\\€]+)[\\s\\:\\!\\?\\;\\…\\€]*){0,"+max.toString()+"}$"},createMaxCharPattern:function(max){return"^[\\s\\S]{0,"+max.toString()+"}$"}}}),define("taoQtiItem/qtiCommonRenderer/helpers/PciPrettyPrint",["lodash"],function(_){"use strict";var _formatters={boolean:function(value){return value?"true":"false"},integer:function(value){return value},float:function(value){return value},string:function(value){return""===value?"NULL":'"'+value+'"'},point:function(value){return"["+value[0]+", "+value[1]+"]"},pair:function(value){return"["+value[0]+", "+value[1]+"]"},directedPair:function(value){return"["+value[0]+", "+value[1]+"]"},
duration:function(value){return value},file:function(value){return"binary data"},uri:function(value){return value},intOrIdentifier:function(value){return value},identifier:function(value){return value}};return{printBase:function(value,withType){var print="",base=value.base;if(withType=void 0===withType||withType,base)return _.forIn(_formatters,function(formatter,baseType){if(void 0!==base[baseType])return print+=withType?"("+baseType+") ":"",print+=formatter(base[baseType]),!1}),print},printList:function(value,withType){var print="",list=value.list;if(withType=void 0===withType||withType,list)return _.forIn(_formatters,function(formatter,baseType){if(
void 0!==list[baseType])return print+=withType?"("+baseType+") ":"",print+="[",_.each(list[baseType],function(value){print+=formatter(value)+", "}),_.size(list[baseType])&&(print=print.substring(0,print.length-2)),print+="]",!1}),print},printRecord:function(value){return value&&value.record?"(record) "+JSON.stringify(value.record):""}}}),define("taoQtiItem/qtiCommonRenderer/helpers/PciResponse",["lodash","taoQtiItem/qtiCommonRenderer/helpers/PciPrettyPrint"],function(_,pciPrettyPrint){"use strict";var _qtiModelPciResponseCardinalities={single:"base",multiple:"list",ordered:"list",record:"record"};return{unserialize:function(response,interaction){
var mappedCardinality,ret=[],responseDeclaration=interaction.getResponseDeclaration(),baseType=responseDeclaration.attr("baseType"),cardinality=responseDeclaration.attr("cardinality");if(!_qtiModelPciResponseCardinalities[cardinality])throw"unknown cardinality in the responseDeclaration of the interaction";var responseValues=response[mappedCardinality=_qtiModelPciResponseCardinalities[cardinality]];if(null===responseValues)ret=[];else{if(!_.isObject(responseValues))throw"invalid response cardinality, expected "+cardinality+" ("+mappedCardinality+")";if(void 0===responseValues[baseType])throw"invalid response baseType";ret=responseValues[baseType],
ret=_.isArray(ret)?ret:[ret]}return ret},serialize:function(responseValues,interaction){if(!_.isArray(responseValues))throw"invalid argument : responseValues must be an Array";var mappedCardinality,response={},responseDeclaration=interaction.getResponseDeclaration(),baseType=responseDeclaration.attr("baseType"),cardinality=responseDeclaration.attr("cardinality");if(responseValues=_.map(responseValues||[],function(v){return"boolean"===baseType?!0===v||"true"===v:v}),!_qtiModelPciResponseCardinalities[cardinality])throw"unknown cardinality in the responseDeclaration of the interaction";return"base"===(
...
)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='>\r\n    <span data-identifier="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" class="gapmatch-content">&nbsp;</span>\r\n</span>'})}),define("taoQtiItem/qtiCommonRenderer/renderers/choices/Gap",["tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/gap","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"gap",getContainer:containerHelper.get,template:tpl}}),define(
"tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/gapImg",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<li class="qti-choice qti-gapImg selectable" data-identifier="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}
):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='>\n    <img src="'+escapeExpression(typeof(stack1=null==(stack1=null==(
stack1=depth0&&depth0.object)||!1===stack1?stack1:stack1.attributes)||!1===stack1?stack1:stack1.data)===functionType?stack1.apply(depth0):stack1)+'" \n         width="'+escapeExpression(typeof(stack1=null==(stack1=null==(stack1=depth0&&depth0.object)||!1===stack1?stack1:stack1.attributes)||!1===stack1?stack1:stack1.width)===functionType?stack1.apply(depth0):stack1)+'"\n         height="'+escapeExpression(typeof(stack1=null==(stack1=null==(stack1=depth0&&depth0.object)||!1===stack1?stack1:stack1.attributes)||!1===stack1?stack1:stack1.height)===functionType?stack1.apply(depth0):stack1)+'"\n         ',((stack1=helpers.if.call(depth0,null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.objectLabel,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+='alt="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.objetLabel)===functionType?stack1.apply(depth0):stack1)+'" '},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="\n    />\n</li>"})}),define("taoQtiItem/qtiCommonRenderer/renderers/choices/GapImg",["tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/gapImg","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"gapImg",
getContainer:containerHelper.get,template:tpl}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/choice",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<li class="qti-choice" data-identifier="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" data-serial="',stack1=(helper=helpers.serial)?helper.call(
depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((stack1=(helper=helpers.body)?helper.call(
depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="\n</li>"})}),define("taoQtiItem/qtiCommonRenderer/renderers/choices/GapText",["tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/choice","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"gapText",getContainer:containerHelper.get,template:tpl}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/hottext",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],
...
"taoQtiItem/qtiCommonRenderer/renderers/choices/SimpleAssociableChoice.MatchInteraction",["tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/simpleAssociableChoice.matchInteraction","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"simpleAssociableChoice.matchInteraction",getContainer:containerHelper.get,template:tpl}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/choices/simpleChoice.choiceInteraction",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,
helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing;return buffer+='<li class="qti-choice qti-simpleChoice" data-identifier="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='>\n    <div class="pseudo-label-box">\n        <label class="real-label">\n            ',((stack1=helpers.if.call(depth0,depth0&&depth0.unique,{hash:{},inverse:this.program(5,function(depth0,data){var stack1,buffer=""
;return buffer+='\n            <input type="checkbox" name="response-'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.interaction)||!1===stack1?stack1:stack1.serial)===functionType?stack1.apply(depth0):stack1)+'" value="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" tabindex="1">\n            <span class="icon-checkbox"></span>\n            '},data),fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+='\n            <input type="radio" name="response-'+escapeExpression(typeof(stack1=null==(
stack1=depth0&&depth0.interaction)||!1===stack1?stack1:stack1.serial)===functionType?stack1.apply(depth0):stack1)+'" value="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" tabindex="1">\n            <span class="icon-radio"></span>\n            '},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n        </label>\n        <div class="label-box">\n            <div class="label-content clear" contenteditable="false">\n                ',((stack1=(helper=helpers.body)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body
...
buffer+='\n    <div class="instruction-container"></div>\n    <div class="image-editor solid">\n        <div id=\'graphic-paper-',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'\' class="main-image-box"></div>\n        <div class="clearfix"></div>\n        <ul class="none block-listing horizontal source">\n            ',options={hash:{},inverse:this.noop,fn:this.program(9,program9,data),data:data},stack1=(helper=helpers.gapImgs)?helper.call(depth0,options):typeof(helper=depth0&&depth0.gapImgs
)===functionType?helper.call(depth0,options):helper,helpers.gapImgs||(stack1=blockHelperMissing.call(depth0,stack1,{hash:{},inverse:this.noop,fn:this.program(9,program9,data),data:data})),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n        </ul>\n    </div>\n</div>\n"})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/GraphicGapMatchInteraction",["jquery","lodash","i18n","module","core/promise","core/mouseEvent","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/graphicGapMatchInteraction","taoQtiItem/qtiCommonRenderer/helpers/Graphic","taoQtiItem/qtiCommonRenderer/helpers/PciResponse","taoQtiItem/qtiCommonRenderer/helpers/container",
...
var element=interaction.paper.getById(choice.serial);element&&(element.selectable=!1,graphic.setStyle(element,"basic"),graphic.updateTitle(element,__("Select an image first")))}),_.forEach(interaction.gapFillers,function(gapFiller){gapFiller.forEach(function(element){graphic.updateTitle(element,__("Remove"))})})},_selectShape=function(interaction,element,trackResponse){var $img,$clone,gapFiller,id,bbox,shapeOffset,activeOffset,matching,currentCount,$container=containerHelper.get(interaction),$gapList=$("ul",$container),$active=$gapList.find(".active:first"),$imageBox=$(".main-image-box",$container),boxOffset=$imageBox.offset();void 0===trackResponse&&(
trackResponse=!0),$active.length&&(id=$active.data("identifier"),(matching=element.data("matching")||[]).push(id),element.data("matching",matching),currentCount=matching.length,$img=$active.find("img"),_shapesUnSelectable(interaction),$gapList.children().removeClass("active"),function(interaction,$choice){var matchMax,usages,choiceSerial=$choice.data("serial"),choice=interaction.getGapImg(choiceSerial);_choiceUsages[choiceSerial]||(_choiceUsages[choiceSerial]=0),_choiceUsages[choiceSerial]++,!interaction.responseMappingMode&&choice.attr("matchMax")&&(matchMax=+choice.attr("matchMax"),usages=+_choiceUsages[choiceSerial],0!==matchMax&&matchMax<=usages&&(
interact($choice.get(0)).draggable(!1),$choice.addClass("disabled"),$choice.removeClass("selectable")))}(interaction,$active),$clone=$img.clone(),shapeOffset=$(element.node).offset(),activeOffset=$active.offset(),$clone.css({position:"absolute",display:"block","z-index":1e4,opacity:.8,top:activeOffset.top-boxOffset.top,left:activeOffset.left-boxOffset.left}),$clone.appendTo($imageBox),$clone.animate({top:shapeOffset.top-boxOffset.top,left:shapeOffset.left-boxOffset.left},200,function(){var gapFillerImage;$clone.remove(),bbox=element.getBBox(),gapFillerImage=(gapFiller=graphic.createBorderedImage(interaction.paper,{url:$img.attr("src"),left:bbox.x+8*(
currentCount-1),top:bbox.y+8*(currentCount-1),width:parseInt($img.attr("width"),10),height:parseInt($img.attr("height"),10),padding:0,border:!1,shadow:!0}).data("identifier",id).toFront())[2].node,interact(gapFillerImage).on("tap",function(e){var $choice,choiceSerial,target=e.currentTarget,rElement=interaction.paper.getById(target.raphaelid);e.preventDefault(),e.stopPropagation(),0<$gapList.find(".active").length?interactUtils.tapOn(element.node):(element.data("matching",_.without(element.data("matching")||[],rElement.data("identifier"))),interaction.gapFillers=_.without(interaction.gapFillers,gapFiller),gapFiller.remove(),choiceSerial=($choice=$active
).data("serial"),_choiceUsages[choiceSerial]--,$choice.removeClass("disabled"),$choice.addClass("selectable"),interact($choice.get(0)).draggable(!0),containerHelper.triggerResponseChangeEvent(interaction))}),interaction.gapFillers.push(gapFiller),containerHelper.triggerResponseChangeEvent(interaction)}))},_renderChoice=function(interaction,choice){var rElement=graphic.createElement(interaction.paper,choice.attr("shape"),choice.attr("coords"),{id:choice.serial,title:__("Select an image first"),hover:!1}).data("max",choice.attr("matchMax")).data("matching",[]);function handleShapeSelect(){_isMatchable(rElement)&&!0===rElement.selectable&&_selectShape(
interaction,rElement)}interact(rElement.node).on("tap",function(){handleShapeSelect()}),isDragAndDropEnabled&&interact(rElement.node).dropzone({overlap:.15,ondragenter:function(){_isMatchable(rElement)&&(graphic.setStyle(rElement,"hover"),activeDrop=rElement.node)},ondrop:function(){_isMatchable(rElement)&&(graphic.setStyle(rElement,"selectable"),handleShapeSelect(),activeDrop=null)},ondragleave:function(){_isMatchable(rElement)&&(graphic.setStyle(rElement,"selectable"),activeDrop=null)}})},_renderGapList=function(interaction,$gapList){var dragOptions,gapFillersSelector=$gapList.selector+" li";function _setActiveGapState($target){$gapList.children("li"
).removeClass("active"),$target.addClass("active"),function(interaction){var tooltip=__("Select the area to add an image");_.forEach(interaction.getChoices(),function(choice){var element=interaction.paper.getById(choice.serial);_isMatchable(element)&&(element.selectable=!0,graphic.setStyle(element,"selectable"),graphic.updateTitle(element,tooltip))}),_.forEach(interaction.gapFillers,function(gapFiller){gapFiller.forEach(function(element){graphic.updateTitle(element,tooltip)})})}(interaction)}function _setInactiveGapState($target){$target.removeClass("active"),_shapesUnSelectable(interaction)}interact(gapFillersSelector).on("tap",function(e){var $target
;e.stopPropagation(),e.preventDefault(),($target=$(e.currentTarget)).hasClass("disabled")||($target.hasClass("active")?_setInactiveGapState($target):_setActiveGapState($target))}),isDragAndDropEnabled&&(dragOptions={inertia:!1,autoScroll:!0,restrict:{restriction:".qti-interaction",endOnly:!1,elementRect:{top:0,left:0,bottom:1,right:1}}},$(gapFillersSelector).each(function(index,gap){interact(gap).draggable(_.assign({},dragOptions,{onstart:function(e){var draggableSelector,target,$target=$(e.target);_setActiveGapState($target),$target.addClass("dragged"),draggableSelector=gapFillersSelector,target=e.target,interactUtils.iFrameDragFixOn(function(){
activeDrop&&interact(activeDrop).fire({type:"drop",target:activeDrop,relatedTarget:target}),interact(draggableSelector).fire({type:"dragend",target:target})})},onmove:function(e){interactUtils.moveElement(e.target,e.dx,e.dy)},onend:function(e){var $target=$(e.target);_setInactiveGapState($target),$target.removeClass("dragged"),interactUtils.restoreOriginalPosition($target),interactUtils.iFrameDragFixOff()}})).styleCursor(!1)}))};return{qtiClass:"graphicGapMatchInteraction",template:tpl,render:function(interaction){var self=this;return new Promise(function(resolve){var $container=containerHelper.get(interaction),$gapList=$("ul.source",$container),
background=interaction.object.attributes;interaction.gapFillers=[],self.getOption&&self.getOption("enableDragAndDrop")&&self.getOption("enableDragAndDrop").graphicGapMatch&&(isDragAndDropEnabled=self.getOption("enableDragAndDrop").graphicGapMatch),$container.off("resized.qti-widget.resolve").one("resized.qti-widget.resolve",resolve),interaction.paper=graphic.responsivePaper("graphic-paper-"+interaction.serial,interaction.serial,{width:background.width,height:background.height,img:self.resolveUrl(background.data),imgId:"bg-image-"+interaction.serial,container:$container,resize:function(newSize,factor){$gapList.css("max-width",newSize+"px"),
1!==factor&&$gapList.find("img").each(function(){var $img=$(this);$img.width($img.attr("width")*factor),$img.height($img.attr("height")*factor)})}}),_.forEach(interaction.getChoices(),_.partial(_renderChoice,interaction)),_renderGapList(interaction,$gapList),function(interaction){var $container=containerHelper.get(interaction),$gapImages=$("ul > li",$container),bgImage=interaction.paper.getById("bg-image-"+interaction.serial);bgImage&&interact(bgImage.node).on("tap",function(){_shapesUnSelectable(interaction),$gapImages.removeClass("active")})}(interaction)})},getContainer:containerHelper.get,setResponse:function(interaction,response){var responseValues,
$container=containerHelper.get(interaction);if(response&&interaction.paper){try{responseValues=pciResponse.unserialize(response,interaction)}catch(e){responseValues=null}_.isArray(responseValues)&&_.forEach(interaction.getChoices(),function(choice){var element=interaction.paper.getById(choice.serial);element&&_.forEach(responseValues,function(pair){var responseChoice,responseGap;2===pair.length&&(responseChoice=isDirectedPairFlipped?pair[0]:pair[1],responseGap=isDirectedPairFlipped?pair[1]:pair[0],responseChoice===choice.id()&&($("[data-identifier="+responseGap+"]",$container).addClass("active"),_selectShape(interaction,element,!1)))})})}},
...
interaction.setResponse(state.response))},getState:function(interaction){var state={},response=interaction.getResponse();return response&&(state.response=response),state},isDirectedPairFlipped:isDirectedPairFlipped}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/graphicOrderInteraction",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+="<div ",((stack1=helpers.if.call(depth0,null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=' class="qti-interaction qti-blockInteraction qti-graphicInteraction qti-graphicOrderInteraction clearfix',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){
var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,
buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))||0===stack1)&&(buffer+=stack1),
...
stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=' class="qti-interaction qti-blockInteraction qti-graphicInteraction qti-hotspotInteraction clearfix',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,
fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,
fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),
data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n    <div class="instruction-container"></div>\n    <div class="image-editor solid">\n        <div id=\'graphic-paper-',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'\' class="main-image-box"></div>\n    </div>\n</div>\n'})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/HotspotInteraction",["jquery","lodash","i18n","core/promise","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/hotspotInteraction",
...
escapeExpression=this.escapeExpression,blockHelperMissing=helpers.blockHelperMissing;function program7(depth0,data){var stack1;return(stack1=typeof depth0===functionType?depth0.apply(depth0):depth0)||0===stack1?stack1:""}return buffer+="<select ",((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),
buffer+=' class="qti-interaction qti-inlineInteraction qti-inlineChoiceInteraction',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='"\n        data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial
...
var responseCount=_.size(_getRawResponse(interaction));choice&&choice.attributes&&0<choice.attributes.matchMax&&_countAssociations(interaction,choice)>choice.attributes.matchMax?onMatchMaxReached(interaction,choice,this,msg,this.getLevel()):responseCount<minAssociations?this.setLevel("info"):maxAssociations<responseCount?onMaxChoicesReached(this,msg):minAssociations<=responseCount&&responseCount<=maxAssociations&&this.setLevel("success")}))};return{qtiClass:"matchInteraction",template:tpl,render:function(interaction){var $container=containerHelper.get(interaction);_setInstructions(interaction),$container.on("click.commonRenderer","input[type=checkbox]",
function(e){_onCheckboxSelected(interaction,e)}),instructionMgr.validateInstructions(interaction)},getContainer:containerHelper.get,setResponse:function(interaction,response){var $container=containerHelper.get(interaction);void 0!==(response=_filterResponse(response)).list&&void 0!==response.list.directedPair&&_(response.list.directedPair).forEach(function(directedPair){var x=$("th[data-identifier="+directedPair[0]+"]",$container).index()-1,y=$("th[data-identifier="+directedPair[1]+"]",$container).parent().index();$(".matrix > tbody tr",$container).eq(y).find("input[type=checkbox]").eq(x).attr("checked",!0)}),instructionMgr.validateInstructions(interaction)
...
hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='"\n     data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"\n     data-qti-class="orderInteraction"\n     data-orientation="',((stack1=helpers.if.call(depth0,
depth0&&depth0.horizontal,{hash:{},inverse:this.program(11,function(depth0,data){return"vertical"},data),fn:this.program(9,program9,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(13,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((
stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(15,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n    <div class="instruction-container"></div>\n    <div class="order-interaction-area">\n        <ul class="choice-area square source solid block-listing ',((stack1=helpers.if.call(depth0,depth0&&depth0.horizontal,{hash:{},inverse:this.noop,fn:this.program(9,
program9,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='">\n            ',options={hash:{},inverse:this.noop,fn:this.program(17,program17,data),data:data},stack1=(helper=helpers.choices)?helper.call(depth0,options):typeof(helper=depth0&&depth0.choices)===functionType?helper.call(depth0,options):helper,helpers.choices||(stack1=blockHelperMissing.call(depth0,stack1,{hash:{},inverse:this.noop,fn:this.program(17,program17,data),data:data})),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n        </ul>\n        <div class="arrow-bar middle">\n            <span class="icon-add-to-selection ',((stack1=helpers.if.call(depth0,depth0&&depth0.horizontal,{
hash:{},inverse:this.program(21,program21,data),fn:this.program(19,program19,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='"></span>\n            <span class="icon-remove-from-selection ',((stack1=helpers.if.call(depth0,depth0&&depth0.horizontal,{hash:{},inverse:this.program(25,program25,data),fn:this.program(23,program23,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=' inactive"></span>\n        </div>\n        <ul class="drag-container block-listing"></ul>\n        <ul class="result-area decimal target solid block-listing ',((stack1=helpers.if.call(depth0,depth0&&depth0.horizontal,{hash:{},inverse:this.noop,fn:this.program(9,
program9,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='">\n        </ul>\n        <div class="arrow-bar">\n            <span class="icon-move-before ',((stack1=helpers.if.call(depth0,depth0&&depth0.horizontal,{hash:{},inverse:this.program(23,program23,data),fn:this.program(25,program25,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=' inactive"></span>\n            <span class="icon-move-after ',((stack1=helpers.if.call(depth0,depth0&&depth0.horizontal,{hash:{},inverse:this.program(19,program19,data),fn:this.program(21,program21,data),data:data}))||0===stack1)&&(buffer+=stack1),
buffer+=' inactive"></span>\n        </div>\n    </div>\n    <div class="notification-container"></div>\n</div>\n'})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/OrderInteraction",["lodash","jquery","i18n","core/mouseEvent","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/orderInteraction","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/instructions/instructionManager","taoQtiItem/qtiCommonRenderer/helpers/PciResponse","interact","ui/interactUtils"],function(_,$,__,triggerMouseEvent,tpl,containerHelper,instructionMgr,pciResponse,interact,interactUtils){"use strict";var _freezeSize=function($container
){var $orderArea=$container.find(".order-interaction-area");$orderArea.height($orderArea.height())},_setInstructions=function(interaction){var $container=containerHelper.get(interaction),$choiceArea=$(".choice-area",$container),$resultArea=$(".result-area",$container),min=parseInt(interaction.attr("minChoices"),10),max=parseInt(interaction.attr("maxChoices"),10);if(min&&instructionMgr.appendInstruction(interaction,__("You must use at least %d choices",min),function(){$resultArea.find(">li").length>=min?this.setLevel("success"):this.reset()}),max&&max<_.size(interaction.getChoices())){var instructionMax=instructionMgr.appendInstruction(interaction,__(
"You can use maximum %d choices",max),function(){$resultArea.find(">li").length>=max?($choiceArea.find(">li").addClass("deactivated"),this.setMessage(__("Maximum choices reached"))):($choiceArea.find(">li").removeClass("deactivated"),this.reset())});interact($choiceArea.selector+" >li.deactivated").on("tap",function(e){var $target=$(e.currentTarget);$target.addClass("brd-error"),instructionMax.setLevel("warning",2e3),setTimeout(function(){$target.removeClass("brd-error")},150)}),interact($choiceArea.selector+" >li.deactivated").draggable({onstart:function(e){$(e.target).addClass("brd-error"),instructionMax.setLevel("warning")},onend:function(e){$(e.target
).removeClass("brd-error"),instructionMax.setLevel("info")}}).styleCursor(!1)}},resetResponse=function(interaction){var $container=containerHelper.get(interaction),initialOrder=_.keys(interaction.getChoices()),$choiceArea=$(".choice-area",$container).append($(".result-area>li",$container)),$choices=$choiceArea.children(".qti-choice");$container.find(".qti-choice.active").each(function(){interactUtils.tapOn(this)}),$choices.detach().sort(function(choice1,choice2){return _.indexOf(initialOrder,$(choice1).data("serial"))>_.indexOf(initialOrder,$(choice2).data("serial"))}),$choiceArea.prepend($choices)};return{qtiClass:"orderInteraction",getData:function(
interaction,data){return _.merge(data||{},{horizontal:"horizontal"===interaction.attr("orientation")})},template:tpl,render:function(interaction){var isDragAndDropEnabled,dragOptions,$dropzoneElement,$container=containerHelper.get(interaction),$choiceArea=$container.find(".choice-area"),$resultArea=$container.find(".result-area"),$iconAdd=$container.find(".icon-add-to-selection"),$iconRemove=$container.find(".icon-remove-from-selection"),$iconBefore=$container.find(".icon-move-before"),$iconAfter=$container.find(".icon-move-after"),$activeChoice=null,choiceSelector=$choiceArea.selector+" >li:not(.deactivated)",resultSelector=$resultArea.selector+" >li",
$dragContainer=$container.find(".drag-container"),orientation=interaction.attr("orientation")?interaction.attr("orientation"):"vertical",_setSelection=function($choice){$activeChoice&&$activeChoice.removeClass("active"),($activeChoice=$choice).addClass("active")},_resetSelection=function(){$activeChoice&&($activeChoice.removeClass("active"),$activeChoice=null),$iconAdd.removeClass("inactive"),$iconRemove.removeClass("active").addClass("inactive"),$iconBefore.removeClass("active").addClass("inactive"),$iconAfter.removeClass("active").addClass("inactive")},_addChoiceToSelection=function($target,position){var $results=$(resultSelector);_resetSelection(),
void 0!==position&&position<$results.length?$results.eq(position).before($target):$resultArea.append($target),containerHelper.triggerResponseChangeEvent(interaction),instructionMgr.validateInstructions(interaction)},_toggleResultSelection=function($target){$target.hasClass("active")?_resetSelection():(_setSelection($target),$iconAdd.addClass("inactive"),$iconRemove.removeClass("inactive").addClass("active"),$iconBefore.removeClass("inactive").addClass("active"),$iconAfter.removeClass("inactive").addClass("active"))},_removeChoice=function(){$activeChoice&&($choiceArea.append($activeChoice),containerHelper.triggerResponseChangeEvent(interaction),
instructionMgr.validateInstructions(interaction)),_resetSelection()};function _iFrameDragFix(draggableSelector,target){interactUtils.iFrameDragFixOn(function(){_isDropzoneVisible()&&interact($resultArea.selector).fire({type:"drop",target:$dropzoneElement.eq(0),relatedTarget:target}),interact(draggableSelector).fire({type:"dragend",target:target})})}function _isDropzoneVisible(){return $.contains($container.get(0),$dropzoneElement.get(0))}function _adjustDropzonePosition($dragged){var draggedBox=$dragged.get(0).getBoundingClientRect(),$prevResult=$dropzoneElement.prev(".qti-choice"),$nextResult=$dropzoneElement.next(".qti-choice"),
prevMiddle=0<$prevResult.length&&_getMiddleOf($prevResult),nextMiddle=0<$nextResult.length&&_getMiddleOf($nextResult);"horizontal"!==orientation?(prevMiddle&&draggedBox.top<prevMiddle.y&&$prevResult.before($dropzoneElement),nextMiddle&&draggedBox.bottom>nextMiddle.y&&$nextResult.after($dropzoneElement)):(prevMiddle&&draggedBox.left<prevMiddle.x&&$prevResult.before($dropzoneElement),nextMiddle&&draggedBox.right>nextMiddle.x&&$nextResult.after($dropzoneElement))}function _getMiddleOf($element){var elementBox=$element.get(0).getBoundingClientRect();return{x:elementBox.left+elementBox.width/2,y:elementBox.top+elementBox.height/2}}interact($container.selector
).on("tap",function(){_resetSelection()}),interact(choiceSelector).on("tap",function(e){var $target=$(e.currentTarget);$target.closest(".qti-item").hasClass("prevent-click-handler")||(e.stopPropagation(),$iconAdd.addClass("triggered"),setTimeout(function(){$iconAdd.removeClass("triggered")},150),_addChoiceToSelection($target))}),interact(resultSelector).on("tap",function(e){var $target=$(e.currentTarget);$target.closest(".qti-item").hasClass("prevent-click-handler")||(e.stopPropagation(),_toggleResultSelection($target))}),interact($iconRemove.selector).on("tap",function(e){$(e.currentTarget).closest(".qti-item").hasClass("prevent-click-handler")||(
e.stopPropagation(),_removeChoice())}),interact($iconBefore.selector).on("tap",function(e){var $prev;$(e.currentTarget).closest(".qti-item").hasClass("prevent-click-handler")||(e.stopPropagation(),($prev=$activeChoice.prev()).length&&($prev.before($activeChoice),containerHelper.triggerResponseChangeEvent(interaction)))}),interact($iconAfter.selector).on("tap",function(e){var $next;$(e.currentTarget).closest(".qti-item").hasClass("prevent-click-handler")||(e.stopPropagation(),($next=$activeChoice.next()).length&&($next.after($activeChoice),containerHelper.triggerResponseChangeEvent(interaction)))}),this.getOption&&this.getOption("enableDragAndDrop"
)&&this.getOption("enableDragAndDrop").order&&(isDragAndDropEnabled=this.getOption("enableDragAndDrop").order),isDragAndDropEnabled&&($dropzoneElement=$("<li>",{class:"dropzone qti-choice"}),$("<div>",{class:"qti-block"}).appendTo($dropzoneElement),dragOptions={inertia:!1,autoScroll:!0,restrict:{restriction:".qti-interaction",endOnly:!1,elementRect:{top:0,left:0,bottom:1,right:1}}},interact(choiceSelector).draggable(_.assign({},dragOptions,{onstart:function(e){$(e.target).addClass("dragged"),_iFrameDragFix(choiceSelector,e.target)},onmove:function(e){var $target=$(e.target);interactUtils.moveElement(e.target,e.dx,e.dy),_isDropzoneVisible(
)&&_adjustDropzonePosition($target)},onend:function(e){var $target=$(e.target);$target.removeClass("dragged"),interactUtils.restoreOriginalPosition($target),interactUtils.iFrameDragFixOff()}})).styleCursor(!1),interact(resultSelector).draggable(_.assign({},dragOptions,{onstart:function(e){var $target=$(e.target);$target.addClass("dragged"),_setSelection($target),$dragContainer.show(),$dragContainer.offset($target.offset()),"horizontal"===orientation?$dragContainer.width($(e.currentTarget).width()):$dragContainer.width($target.parent().width()),$dragContainer.append($target),_iFrameDragFix(resultSelector,e.target)},onmove:function(e){var $target=$(e.target)
;interactUtils.moveElement(e.target,e.dx,e.dy),_isDropzoneVisible()&&_adjustDropzonePosition($target)},onend:function(e){var $target=$(e.target),hasBeenDroppedInResultArea=$target.parent===$resultArea;$target.removeClass("dragged"),$dragContainer.hide(),hasBeenDroppedInResultArea||_removeChoice(),interactUtils.restoreOriginalPosition($target),interactUtils.iFrameDragFixOff()}})).styleCursor(!1),interact($resultArea.selector).dropzone({overlap:.5,ondragenter:function(e){var $dragged=$(e.relatedTarget);!function($dragged){var insertPosition,draggedMiddle=_getMiddleOf($dragged),previousMiddle={x:0,y:0};$(resultSelector).each(function(index){
var currentMiddle=_getMiddleOf($(this));if("horizontal"!==orientation){if(draggedMiddle.y>previousMiddle.y&&draggedMiddle.y<currentMiddle.y)return insertPosition=index,!1;previousMiddle.y=currentMiddle.y}else{if(draggedMiddle.x>previousMiddle.x&&draggedMiddle.x<currentMiddle.x)return insertPosition=index,!1;previousMiddle.x=currentMiddle.x}}),void 0!==insertPosition?$(resultSelector).eq(insertPosition).before($dropzoneElement):$resultArea.append($dropzoneElement),$dropzoneElement.height($dragged.height()),$dropzoneElement.find("div").text($dragged.text())}($dragged),$dragged.addClass("droppable")},ondrop:function(e){var $dragged=$(e.relatedTarget),
dropzoneIndex=$(resultSelector).index($dropzoneElement);this.ondragleave(e),_addChoiceToSelection($dragged,dropzoneIndex),interactUtils.restoreOriginalPosition($dragged)},ondragleave:function(e){var $dragged=$(e.relatedTarget);$dropzoneElement.remove(),$dragged.removeClass("droppable")}})),_setInstructions(interaction),$(document).on("attributeChange.qti-widget.commonRenderer",function(e,data){data.element.getSerial()===interaction.getSerial()&&("maxChoices"!==data.key&&"minChoices"!==data.key||(instructionMgr.removeInstructions(interaction),_setInstructions(interaction),instructionMgr.validateInstructions(interaction)))}),_freezeSize($container)},
...
"taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/instanciator",["qtiCustomInteractionContext"],function(qtiCustomInteractionContext){"use strict";return{getPci:function(interaction){var pciTypeIdentifier,pci=interaction.data("pci");if(!pci){if(pciTypeIdentifier=interaction.typeIdentifier,!(pci=qtiCustomInteractionContext.createPciInstance(pciTypeIdentifier)))throw new Error("no custom interaction hook found for the type "+pciTypeIdentifier);interaction.data("pci",pci),pci._taoCustomInteraction=interaction}return pci}}}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/common",["lodash","taoQtiItem/qtiCommonRenderer/helpers/container",
"taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/instanciator"],function(_,containerHelper,instanciator){"use strict";return function(runtime){return{getRequiredModules:function(){var requireEntries=[];return runtime.hook&&requireEntries.push(runtime.hook.replace(/\.js$/,"")),_.forEach(runtime.libraries,function(lib){requireEntries.push(lib.replace(/\.js$/,""))}),_.forEach(runtime.stylesheets,function(stylesheet){requireEntries.push("css!"+stylesheet.replace(/\.css$/,""))}),requireEntries},createInstance:function(interaction,context){var id=interaction.attr("responseIdentifier"),pci=instanciator.getPci(interaction),properties=_.clone(
interaction.properties),assetManager=context.assetManager,pciAssetManager={resolve:function(url){var resolved=assetManager.resolveBy("portableElementLocation",url);return resolved===url||_.isUndefined(resolved)?assetManager.resolve(url):resolved}};pci.initialize(id,containerHelper.get(interaction).children().get(0),properties,pciAssetManager)},destroy:function(interaction){return instanciator.getPci(interaction).destroy()},setState:function(interaction,state){instanciator.getPci(interaction).setSerializedState(state)},getState:function(interaction){return instanciator.getPci(interaction).getSerializedState()}}}}),define(
"taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/ims",["lodash","core/logger","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/instanciator"],function(_,loggerFactory,containerHelper,instanciator){"use strict";var logger=loggerFactory("taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/ims"),pciReadyCallback=function(pci,state){logger.info("pciReadyCallback called on PCI "+pci.typeIdentifier)},pciDoneCallback=function(pci,response,state,status){logger.info("pciDoneCallback called on PCI "+pci.typeIdentifier)};return function(runtime){return{getRequiredModules:function(){
var requireEntries=[];return _.forEach(runtime.modules,function(module,name){requireEntries.push(name)}),requireEntries},createInstance:function(interaction,context){var config,pci=instanciator.getPci(interaction),properties=_.clone(interaction.properties);_.forOwn(properties,function(propVal,propKey){properties[propKey]=_.isArray(propVal)||_.isObject(propVal)?JSON.stringify(propVal):propVal}),config={properties:properties,templateVariables:{},boundTo:context.response||{},onready:pciReadyCallback,ondone:pciDoneCallback,status:"interacting"},pci.getInstance(containerHelper.get(interaction).get(0),config,context.state)},destroy:function(interaction){
instanciator.getPci(interaction).oncompleted()},setState:_.noop,getState:function(interaction){return instanciator.getPci(interaction).getState()}}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/customInteraction",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="qti-interaction qti-customInteraction" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}
):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((stack1=(helper=helpers.markup)?helper.call(depth0,{hash:{},data:data}
):typeof(helper=depth0&&depth0.markup)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="\n</div>"})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/PortableCustomInteraction",["lodash","core/promise","core/logger","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/customInteraction","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/PortableElement","taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/instanciator","taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/common","taoQtiItem/qtiCommonRenderer/renderers/interactions/pci/ims",
"qtiCustomInteractionContext","taoQtiItem/qtiItem/helper/util","taoQtiItem/portableElementRegistry/ciRegistry"],function(_,Promise,loggerFactory,tpl,containerHelper,PortableElement,instanciator,commonPciRenderer,imsPciRenderer,qtiCustomInteractionContext,util,ciRegistry){"use strict";var _getPciRenderer=function(interaction){return interaction.data("pci-renderer")};return{qtiClass:"customInteraction",template:tpl,getData:function(customInteraction,data){var markup=data.markup;return markup=util.removeMarkupNamespaces(markup),markup=PortableElement.fixMarkupMediaSources(markup,this),data.markup=markup,data},render:function(interaction,options){var self=this
;return options=options||{},new Promise(function(resolve,reject){var state,id=interaction.attr("responseIdentifier"),typeIdentifier=interaction.typeIdentifier,assetManager=self.getAssetManager(),response={};options.state&&options.state[id]&&(state=options.state[id]),response[id]={base:null},ciRegistry.loadRuntimes({include:[typeIdentifier]}).then(function(){var pciRenderer,runtime=ciRegistry.getRuntime(typeIdentifier);if(!runtime)return reject("The runtime for the pci cannot be found : "+typeIdentifier);!function(interaction,runtime){var pciRenderer;pciRenderer="IMSPCI"===runtime.model?imsPciRenderer(runtime):commonPciRenderer(runtime),interaction.data(
"pci-model",runtime.model),interaction.data("pci-renderer",pciRenderer)}(interaction,runtime),pciRenderer=_getPciRenderer(interaction),require(pciRenderer.getRequiredModules(),function(){var pci=instanciator.getPci(interaction);return pci?(pciRenderer.createInstance(interaction,{response:response,state:state,assetManager:assetManager}),_.isFunction(pci.on)&&interaction.onPci("responseChange",function(){containerHelper.triggerResponseChangeEvent(interaction)}),resolve()):reject('Unable to initialize pci "'+id+'"')},reject)}).catch(function(error){reject('Error loading runtime "'+id+'": '+error)})})},getContainer:containerHelper.get,setResponse:function(
interaction,response){instanciator.getPci(interaction).setResponse(response)},getResponse:function(interaction){return instanciator.getPci(interaction).getResponse()},resetResponse:function(interaction){instanciator.getPci(interaction).resetResponse()},destroy:function(interaction){return _getPciRenderer(interaction).destroy(interaction)},getState:function(interaction){return _getPciRenderer(interaction).getState(interaction)},setState:function(interaction,serializedState){_getPciRenderer(interaction).setState(interaction,serializedState)}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/prompt",["handlebars"],function(hb){return hb.template(
function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="qti-prompt-container" data-html-editable-container',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"]
)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='>\n    <div class="qti-prompt" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-html-editable>',((stack1=(helper=helpers.body)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="</div>\n</div>\n"})}),define(
"taoQtiItem/qtiCommonRenderer/renderers/interactions/Prompt",["tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/prompt","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"prompt",template:tpl,getContainer:containerHelper.get}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/selectPointInteraction",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression
;return buffer+="<div ",((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=' class="qti-interaction qti-blockInteraction qti-graphicInteraction qti-selectPointInteraction clearfix',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{
hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},
inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper
)||0===stack1?stack1:""},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n    <div class="instruction-container"></div>\n    <div class="image-editor solid">\n        <div id=\'graphic-paper-',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'\' class="main-image-box"></div>\n    </div>\n</div>\n'})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/SelectPointInteraction",["jquery","lodash","i18n","core/promise",
...
"responsive"),image=interaction.paper.getById("bg-image-"+interaction.serial);interaction.paper.isTouch=!1,image.touchstart(function(){interaction.paper.isTouch=!0,image.untouchstart()}),image.click(function(event){addPoint(interaction,graphic.getPoint(event,interaction.paper,$imageBox,isResponsive))})}(interaction),instructionMgr.minMaxChoiceInstructions(interaction,{min:interaction.attr("minChoices"),max:interaction.attr("maxChoices"),choiceCount:!1,getResponse:getRawResponse,onError:function(data){data&&graphic.highlightError(data.target,"success")}})})},getContainer:containerHelper.get,setResponse:function(interaction,response){var responseValues;if(
response&&interaction.paper)try{responseValues=pciResponse.unserialize(response,interaction),"single"===interaction.getResponseDeclaration().attr("cardinality")&&(responseValues=[responseValues]),_(responseValues).filter(function(point){return _.isArray(point)&&2===point.length}).forEach(function(point){addPoint(interaction,{x:point[0],y:point[1]})})}catch(err){return}},getResponse:function(interaction){return pciResponse.serialize(getRawResponse(interaction),interaction)},resetResponse:function(interaction){interaction&&interaction.paper&&(interaction.paper.points=[],interaction.paper.forEach(function(element){"object"==typeof element.data("point"
)&&graphic.trigger(element,"click")}))},destroy:function(interaction){var $container;interaction.paper&&($container=containerHelper.get(interaction),$(window).off("resize.qti-widget."+interaction.serial),$container.off("resize.qti-widget."+interaction.serial),interaction.paper.clear(),instructionMgr.removeInstructions(interaction),$(".main-image-box",$container).empty().removeAttr("style"),$(".image-editor",$container).removeAttr("style")),containerHelper.reset(interaction)},setState:function(interaction,state){_.isObject(state)&&state.response&&(interaction.resetResponse(),interaction.setResponse(state.response))},getState:function(interaction){var state={}
...
data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="sliderInteraction"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"]
)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=" >\r\n    ",((stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\r\n    <div class="instruction-container"></div>\r\n</div>\r\n'})}),define(
...
;return buffer+='<div class="qti-interaction qti-blockInteraction qti-uploadInteraction" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"]
)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n\t",((stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))||0===stack1)&&(buffer+=stack1),
buffer+='\n\t<div class="instruction-container"></div>\n    <div class="file-upload fixed-grid-row lft">\n        <div class="progressbar"></div>\n        <span class="btn-info small col-4"></span>\n        <span class="file-name placeholder col-8 truncate"></span>\n        <input type="file" ',((stack1=helpers.if.call(depth0,depth0&&depth0.accept,{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,helper,buffer="";return buffer+='accept="',stack1=(helper=helpers.accept)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.accept)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(
stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='/>\n    </div>\n    <div class="file-upload-preview lft visible-file-upload-preview runtime-visible-file-upload-preview">\n        <p class="nopreview">'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"No preview available",options):helperMissing.call(depth0,"__","No preview available",options)))+'</p>\n    </div>\n    <div class="file-upload-preview-popup modal">\n        <div class="modal-body">\n        </div>\n    </div>\n</div>\n'})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/UploadInteraction",["jquery",
"lodash","i18n","context","core/mimetype","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/uploadInteraction","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/instructions/instructionManager","taoQtiItem/qtiCommonRenderer/helpers/uploadMime","ui/progressbar","ui/previewer","ui/modal","ui/waitForMedia"],function($,_,__,context,mimetype,tpl,containerHelper,instructionMgr,uploadHelper){"use strict";var _initialInstructions=__("Browse your computer and select the appropriate file."),_readyInstructions=__("The selected file is ready to be sent."),_handleSelectedFiles=function(interaction,file,messageWrongType){var reader,
$container=containerHelper.get(interaction),filename=file.name,filetype=mimetype.getMimeType(file);if(instructionMgr.removeInstructions(interaction),instructionMgr.appendInstruction(interaction,_initialInstructions),!function(file,interaction){var expectedTypes=uploadHelper.getExpectedTypes(interaction,!0),filetype=mimetype.getMimeType(file);return!expectedTypes.length||0<=_.indexOf(expectedTypes,filetype)}(file,interaction))return instructionMgr.removeInstructions(interaction),instructionMgr.appendInstruction(interaction,function(interaction,userSelectedType,messageWrongType){var types=uploadHelper.getExpectedTypes(interaction),expectedTypeLabels=_.map(
_.uniq(types),function(type){var mime=_.find(uploadHelper.getMimeTypes(),{mime:type});return mime?mime.label:type});return messageWrongType&&_.isFunction(messageWrongType)?messageWrongType({userSelectedType:userSelectedType,types:expectedTypeLabels}):__('Wrong type of file. Expected %s. The selected file has the mimetype "%s".',expectedTypeLabels.join(__(" or ")),userSelectedType)}(interaction,filetype,messageWrongType),function(){this.setLevel("error")}),void instructionMgr.validateInstructions(interaction);$container.find(".file-name").empty().append(filename),(reader=new FileReader).onload=function(e){var base64Data,commaPosition,base64Raw,$previewArea
;instructionMgr.removeInstructions(interaction),instructionMgr.appendInstruction(interaction,_readyInstructions,function(){this.setLevel("success")}),instructionMgr.validateInstructions(interaction),$container.find(".progressbar").progressbar("value",100),commaPosition=(base64Data=e.target.result).indexOf(","),base64Raw=base64Data.substring(commaPosition+1),interaction.data("_response",{base:{file:{data:base64Raw,mime:filetype,name:filename}}}),($previewArea=$container.find(".file-upload-preview")).previewer({url:base64Data,name:filename,mime:filetype}),$previewArea.waitForMedia(function(){var imgNaturalWidth,isOversized,modalWidth,
$originalImg=$previewArea.find("img"),$largeDisplay=$(".file-upload-preview-popup"),itemWidth=$(".qti-item").width(),winWidth=$(window).width()-80,fullHeight=$("body").height();$originalImg.length&&(imgNaturalWidth=$originalImg[0].naturalWidth,isOversized=itemWidth<imgNaturalWidth,modalWidth=Math.min(winWidth,imgNaturalWidth),$previewArea.toggleClass("clickable",isOversized),isOversized&&$previewArea.on("click",function(){$(".upload-ia-modal-bg").remove(),$largeDisplay.find(".modal-body").empty().append($originalImg.clone()),$largeDisplay.on("opened.modal",function(){$(".tao-item-scope.tao-preview-scope").css("overflow","hidden"),$largeDisplay.css({
width:modalWidth,height:fullHeight,left:(modalWidth-itemWidth-40)/-2})}).on("closed.modal",function(){$(".tao-item-scope.tao-preview-scope").css("overflow","auto")}).modal({modalOverlayClass:"modal-bg upload-ia-modal-bg"})}))})},reader.onloadstart=function(){instructionMgr.removeInstructions(interaction),$container.find(".progressbar").progressbar("value",0)},reader.onprogress=function(e){var percentProgress=Math.ceil(Math.round(e.loaded)/Math.round(e.total)*100);$container.find(".progressbar").progressbar("value",percentProgress)},reader.readAsDataURL(file)},_resetGui=function(interaction){var $container=containerHelper.get(interaction);$container.find(
".file-name").text(__("No file selected")),$container.find(".btn-info").text(__("Browse..."))};return{qtiClass:"uploadInteraction",template:tpl,render:function(interaction){var changeListener,$input,self=this,$container=containerHelper.get(interaction);if(_resetGui(interaction),instructionMgr.appendInstruction(interaction,_initialInstructions),interaction.data("_response",{base:null}),changeListener=function(e){var file=e.target.files[0];void 0!==file&&_handleSelectedFiles(interaction,file,self.getCustomMessage("upload","wrongType"))},$input=$container.find("input"),$container.find(".progressbar").progressbar(),!window.FileReader)throw new Error(
"FileReader API not supported! Please use a compliant browser!");$input.bind("change",changeListener),$input.bind("mousedown",function(e){return e.preventDefault(),$(this).blur(),!1})},getContainer:containerHelper.get,setResponse:function(interaction,response){var filename,$container=containerHelper.get(interaction);null!==response.base&&(filename=void 0!==response.base.file.name?response.base.file.name:"previously-uploaded-file",$container.find(".file-name").empty().text(filename)),interaction.data("_response",response)},getResponse:function(interaction){return interaction.data("_response")},resetResponse:function(interaction){_resetGui(interaction)},
destroy:function(interaction){$(document).off(".commonRenderer"),containerHelper.get(interaction).off(".commonRenderer"),instructionMgr.removeInstructions(interaction),containerHelper.reset(interaction)},setState:function(interaction,state){_.isObject(state)&&state.response&&(interaction.resetResponse(),interaction.setResponse(state.response))},getState:function(interaction){var state={},response=interaction.getResponse();return response&&(state.response=response),state},getData:function(interaction,data){return _.merge(data||{},{accept:uploadHelper.getExpectedTypes(interaction,!0).join(",")})},resetGui:_resetGui}}),define(
"tpl!taoQtiItem/qtiCommonRenderer/tpl/item",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="qti-item tao-scope runtime" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-identifier="'+escapeExpression(typeof(stack1=null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    <div",((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1.id,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=' id="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.id)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=' class="qti-itemBody ',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='">',((stack1=(helper=helpers.body)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+='</div>\n    <div id="modalFeedbacks"></div>\n</div>\n'})}),define("taoQtiItem/qtiCommonRenderer/renderers/Item",["tpl!taoQtiItem/qtiCommonRenderer/tpl/item","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/itemStylesheetHandler"],function(tpl,containerHelper,
itemStylesheetHandler){"use strict";return{qtiClass:"assessmentItem",template:tpl,getContainer:containerHelper.get,render:function(item){containerHelper.targetBlank(containerHelper.get(item)),itemStylesheetHandler.attach(item.stylesheets)},destroy:function(item){containerHelper.clear(),item.stylesheets&&itemStylesheetHandler.detach(item.stylesheets)}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/math",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,functionType="function",
escapeExpression=this.escapeExpression,self=this;function program2(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'}return(stack1=helpers.if.call(depth0,depth0&&depth0.block,{hash:{},inverse:self.program(4,function(depth0,data){var stack1,helper,buffer="";return buffer+='<span data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1
)+'" data-qti-class="math"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="><math>",((stack1=(helper=helpers.raw)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.raw)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="</math></span>"},data),fn:self.program(1,function(depth0,data){var stack1,helper,buffer="";return buffer+='<span data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}
):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="math"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='><math display="block">',((stack1=(helper=helpers.raw)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.raw)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="</math></span>"},data),data:data})
)||0===stack1?stack1:""})}),define("taoQtiItem/qtiCommonRenderer/renderers/Math",["lodash","core/promise","tpl!taoQtiItem/qtiCommonRenderer/tpl/math","taoQtiItem/qtiCommonRenderer/helpers/container","mathJax"],function(_,Promise,tpl,containerHelper,MathJax){"use strict";return void 0!==MathJax&&MathJax&&(MathJax.Hub.processSectionDelay=0),{qtiClass:"math",template:tpl,getContainer:containerHelper.get,render:function(math){return new Promise(function(resolve){void 0!==MathJax&&MathJax?(window.MathJax||(window.MathJax=MathJax),_.defer(function(){MathJax.Hub.Queue(["Typeset",MathJax.Hub,containerHelper.get(math).parent()[0]]),MathJax.Hub.Queue(resolve)})
):resolve()})}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/modalFeedback",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div id="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{
hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-identifier="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.identifier)===functionType?stack1.apply(depth0):stack1)+'" class="',((stack1=helpers.unless.call(depth0,depth0&&depth0.inline,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){return"modal "},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="qti-modalFeedback ",stack1=(helper=helpers.feedbackStyle)?helper.call(depth0,{hash:{},data:data}):typeof(
helper=depth0&&depth0.feedbackStyle)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1.title,{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+='<h2 class="qti-title modal-title">'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.title)===functionType?stack1.apply(depth0):stack1)+"</h2>"},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n    <div class="modal-body">',((stack1=(helper=helpers.body)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="</div>\n</div>"})}),define(
"taoQtiItem/qtiItem/helper/container",["lodash","jquery"],function(_,$){"use strict";var _prefix="x-tao-";function _checkContainerType(element){if(_.isFunction(element.initContainer)&&_.isFunction(element.body))return!0;throw"the element is not of a container type"}function _getBodyDom(element){if(_checkContainerType(element))return $("<div>").html(element.body()).find(".x-tao-wrapper")}function _setBodyDomClass(element,newClass,oldClass){if(_checkContainerType(element)&&(oldClass||newClass)){var $wrapper=$("<div>").html(element.body());_setDomClass($wrapper,newClass,oldClass),element.body($wrapper.html())}}function _setDomClass($wrapper,newClass,oldClass){
var $bodyDom=$wrapper.find(".x-tao-wrapper");$bodyDom.length||($wrapper.wrapInner('<div class="x-tao-wrapper">'),$bodyDom=$wrapper.find(".x-tao-wrapper")),oldClass&&$bodyDom.removeClass(oldClass),newClass&&$bodyDom.addClass(newClass)}function _getEncodedDataString(dataName,value){return dataName&&value?_prefix+dataName+"-"+value:""}function getEncodedData(element,dataName){var regex,matches,$body=_getBodyDom(element);if(dataName&&$body&&$body.length&&$body.attr("class")&&(regex=new RegExp(_prefix+dataName+"-([a-zA-Z0-9-._]*)"),matches=$body.attr("class").match(regex)))return matches[1]}return{setEncodedData:function(element,dataName,newValue){
var oldValue=getEncodedData(element,dataName);return _setBodyDomClass(element,_getEncodedDataString(dataName,newValue),_getEncodedDataString(dataName,oldValue))},hasEncodedData:function(element,dataName,value){var $body=_getBodyDom(element);return!!($body&&$body.length&&dataName&&value)&&$body.hasClass(_getEncodedDataString(dataName,value))},getEncodedData:getEncodedData,removeEncodedData:function(element,dataName){var oldValue=getEncodedData(element,dataName);dataName&&oldValue&&_setBodyDomClass(element,"",_getEncodedDataString(dataName,oldValue))},setEncodedDataToDom:function($wrapper,dataName,newValue,oldValue){_setDomClass($wrapper,_getEncodedDataString(
dataName,newValue),_getEncodedDataString(dataName,oldValue))}}}),define("taoQtiItem/qtiCommonRenderer/renderers/ModalFeedback",["lodash","tpl!taoQtiItem/qtiCommonRenderer/tpl/modalFeedback","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiItem/helper/container","ui/waitForMedia","ui/modal"],function(_,tpl,containerHelper,coreContainerHelper){"use strict";var modalFeedbackRenderer={qtiClass:"modalFeedback",template:tpl,getContainer:containerHelper.get,minHeight:200,width:600,getData:function(fb,data){var feedbackStyle=coreContainerHelper.getEncodedData(fb,"modalFeedback");return data.feedbackStyle=feedbackStyle,data},render:function(
modalFeedback,data){data=data||{};var $modal=containerHelper.get(modalFeedback);$modal.waitForMedia(function(){$modal.on("opened.modal",function(){var $itemBody=containerHelper.get(modalFeedback.getRootElement()).children(".qti-itemBody"),requiredHeight=$modal.outerHeight()+parseInt($modal.css("top"));requiredHeight>$itemBody.height()&&$itemBody.height(requiredHeight)}).on("closed.modal",function(){_.isFunction(data.callback)&&data.callback.call(this)}).modal({startClosed:!1,minHeight:modalFeedbackRenderer.minHeight,width:modalFeedbackRenderer.width})})}};return modalFeedbackRenderer}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/object",["handlebars"],
function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="qti-object-container" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},
inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=" >\n</div>\n"})}),define("taoQtiItem/qtiCommonRenderer/renderers/Object",["tpl!taoQtiItem/qtiCommonRenderer/tpl/object","taoQtiItem/qtiCommonRenderer/helpers/container","ui/previewer"],function(tpl,containerHelper){"use strict";return{qtiClass:"object",template:tpl,getContainer:containerHelper.get,render:function(obj){
var $container=containerHelper.get(obj),previewOptions={url:obj.renderer.resolveUrl(obj.attr("data")),mime:obj.attr("type")};obj.attr("height")&&(previewOptions.height=obj.attr("height")),obj.attr("width")&&(previewOptions.width=obj.attr("width")),previewOptions.url&&previewOptions.mime&&$container.previewer(previewOptions)}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/infoControl",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",
escapeExpression=this.escapeExpression;return buffer+='<div class="qti-infoControl',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,
{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n    ",((stack1=(helper=helpers.markup)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.markup)===functionType?helper.call(depth0,{
...
throw"no portable info control hook found with the id "+typeIdentifier}}}),define("taoQtiItem/qtiCommonRenderer/renderers/PortableInfoControl",["lodash","core/promise","tpl!taoQtiItem/qtiCommonRenderer/tpl/infoControl","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/PortableElement","qtiInfoControlContext","taoQtiItem/qtiItem/helper/util","taoQtiItem/portableElementRegistry/icRegistry"],function(_,Promise,tpl,containerHelper,PortableElement,qtiInfoControlContext,util,icRegistry){"use strict";var _getPic=function(infoControl){var typeIdentifier,pic=infoControl.data("pic")||void 0;if(!pic){if(
typeIdentifier=infoControl.typeIdentifier,!(pic=qtiInfoControlContext.createPciInstance(typeIdentifier)))throw"no custom infoControl hook found for the type "+typeIdentifier;infoControl.data("pic",pic),pic._taoInfoControl=infoControl}return pic};return{qtiClass:"infoControl",template:tpl,getData:function(infoControl,data){var markup=data.markup;return markup=util.removeMarkupNamespaces(markup),markup=PortableElement.fixMarkupMediaSources(markup,this),data.markup=markup,data},render:function(infoControl,options){var self=this;return options=options||{},new Promise(function(resolve,reject){var state={},id=infoControl.attr("id"),
typeIdentifier=infoControl.typeIdentifier,config=infoControl.properties,$dom=containerHelper.get(infoControl).children(),assetManager=self.getAssetManager();icRegistry.loadRuntimes().then(function(){var requireEntries=[],runtime=icRegistry.getRuntime(typeIdentifier);if(!runtime)return reject("The runtime for the pic cannot be found : "+typeIdentifier);runtime.hook&&requireEntries.push(runtime.hook.replace(/\.js$/,"")),_.forEach(runtime.libraries,function(module){requireEntries.push(module.replace(/\.js$/,""))}),_.forEach(runtime.stylesheets,function(stylesheet){requireEntries.push("css!"+stylesheet.replace(/\.css$/,""))}),require(requireEntries,function(){
var pic=_getPic(infoControl),picAssetManager={resolve:function(url){var resolved=assetManager.resolveBy("portableElementLocation",url);return resolved===url?assetManager.resolveBy("baseUrl",url):resolved}};return pic?(pic.initialize(id,$dom[0],config,picAssetManager),pic.setSerializedState(state),resolve()):reject("Unable to initialize pic : "+id)},reject)}).catch(function(error){reject("Error loading runtime : "+id)})})},getContainer:containerHelper.get,destroy:function(infoControl){_getPic(infoControl).destroy()},getState:function(infoControl){return _getPic(infoControl).getSerializedState()},setState:function(infoControl,state){_getPic(infoControl
).setSerializedState(state)}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/printedVariable",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<span data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="printedVariable"',((
...
var success=!1;return!1===_locations[qtiClass]?success=!(_renderers[qtiClass]=!1):_locations[qtiClass]&&(list.push(_locations[qtiClass]),success=!0),success}this.setOption=function(key,value){return"string"==typeof key&&(options[key]=value),this},this.setOptions=function(opts){return options=_.extend(options,opts),this},this.getOption=function(key){return"string"==typeof key&&options[key]?options[key]:null},this.getCustomMessage=function(elementName,messageKey){var messages=this.getOption("messages");return messages&&elementName&&messages[elementName]&&_.isString(messages[elementName][messageKey])?Handlebars.compile(messages[elementName][messageKey]):null},
...
;_.isFunction(renderer.setState)?renderer.setState.call(this,qtiInteraction,state):(renderer.resetResponse.call(this,qtiInteraction),renderer.setResponse.call(this,qtiInteraction,state))},this.destroy=function(qtiInteraction,qtiSubclass){var ret=!1,qtiClass=qtiSubclass||qtiInteraction.qtiClass,renderer=_getClassRenderer(qtiClass);if(!renderer)throw"no renderer registered under the name : "+qtiClass;return _.isFunction(renderer.destroy)&&(ret=renderer.destroy.call(this,qtiInteraction)),ret},this.getLoadedRenderers=function(){return _renderers},this.register=function(renderersLocations){_.extend(_locations,renderersLocations)},this.load=function(callback,
requiredClasses){var self=this,required=[],themeData=themesHelper.getCurrentThemeData();if(themeData&&(options.themes=themeData),options.themes&&(options.themes.base=this.resolveUrl(options.themes.base),_.forEach(options.themes.available,function(theme,index){options.themes.available[index].path=self.resolveUrl(theme.path)}),this.themeLoader=themeLoader(options.themes).load(options.preload)),requiredClasses){if(!_.isArray(requiredClasses))throw new Error('invalid argument type: expected array for arg "requireClasses"');requiredClasses=_.intersection(requiredClasses,_renderableClasses),_.each(requiredClasses,function(reqClass){var deps=_dependencies[reqClass]
;deps&&(requiredClasses=_.union(requiredClasses,deps))}),_.forEach(requiredClasses,function(qtiClass){var requiredSubClasses;if(_renderableSubclasses[qtiClass])requiredSubClasses=_.intersection(requiredClasses,_renderableSubclasses[qtiClass]),_.each(requiredSubClasses,function(subclass){if(!registerRendererClass(qtiClass+"."+subclass,required)&&!registerRendererClass(qtiClass,required))throw new Error(self.name+" : missing qti class location declaration: "+qtiClass+", subclass: "+subclass)});else if(!registerRendererClass(qtiClass,required))throw new Error(self.name+" : missing qti class location declaration: "+qtiClass)})}else required=_.values(_locations)
;return require(required,function(){_.each(arguments,function(clazz){(function(renderer){var valid=!0;if("object"!=typeof renderer)return!1;var classCorrect=!1;if(renderer.qtiClass)if(0<=_.indexOf(_renderableClasses,renderer.qtiClass))classCorrect=!0;else{var pos=renderer.qtiClass.indexOf(".");if(0<pos){var qtiClass=renderer.qtiClass.slice(0,pos),subClass=renderer.qtiClass.slice(pos+1);_renderableSubclasses[qtiClass]&&0<=_.indexOf(_renderableSubclasses[qtiClass],subClass)&&(classCorrect=!0)}}if(!classCorrect)throw valid=!1,new Error("invalid qti class name in renderer declaration : "+renderer.qtiClass);if(!renderer.template)throw valid=!1,new Error(
"missing template in renderer declaration : "+renderer.qtiClass);return valid})(clazz)&&(_renderers[clazz.qtiClass]=clazz)}),"function"==typeof callback&&callback.call(self,_renderers)}),this},this.unload=function(){return this.themeLoader&&themeLoader(options.themes).unload(),this},this.setShuffledChoices=function(interaction,choices,identificationType){Element.isA(interaction,"interaction")&&(this.shuffledChoices[interaction.getSerial()]=_.pluck(interactionHelper.findChoices(interaction,choices,identificationType),"serial"))},this.getShuffledChoices=function(interaction,reshuffle,returnedType){var serial,i,choices=[];if(Element.isA(interaction,
"interaction")){if(serial=interaction.getSerial(),!this.shuffledChoices[serial]||reshuffle)if(Element.isA(interaction,"matchInteraction"))for(this.shuffledChoices[serial]=[],i=0;i<2;i++)choices[i]=interactionHelper.shuffleChoices(interaction.getChoices(i)),this.shuffledChoices[serial][i]=_.pluck(choices[i],"serial");else choices=interactionHelper.shuffleChoices(interaction.getChoices()),this.shuffledChoices[serial]=_.pluck(choices,"serial");else Element.isA(interaction,"matchInteraction")?_.forEach(choices,function(choice,index){index<2&&_.forEach(this.shuffledChoices[serial][i],function(choiceSerial){choice.push(interaction.getChoice(choiceSerial))})}
...
;if(/^http(s)?:\/\//i.test(relUrl)||/^data:[^\/]+\/[^;]+(;charset=[\w]+)?;base64,/.test(relUrl))return relUrl;var absUrl="",runtimeLocations=this.getOption("runtimeLocations");return runtimeLocations&&_.size(runtimeLocations)&&_.forIn(runtimeLocations,function(runtimeLocation,typeIdentifier){if(0===relUrl.indexOf(typeIdentifier))return absUrl=relUrl.replace(typeIdentifier,runtimeLocation),!1}),absUrl||(this.getOption("baseUrl")||window.location.protocol+"//"+window.location.host+window.location.pathname.replace(/([^\/]*)$/,""))+relUrl},this.setAreaBroker=function(areaBroker){this._areaBroker=areaBroker},this.getAreaBroker=function(){if(this._areaBroker
)return this._areaBroker},this.getItemCreator=function(){return this.getOption("itemCreator")}};return{build:function(renderersLocations,name,defaultOptions){var NewRenderer=function(){var options=_.isPlainObject(arguments[0])?arguments[0]:{};Renderer.apply(this),this.register(renderersLocations),this.name=name||"",this.setOptions(_.defaults(options,defaultOptions||{}))};return NewRenderer.prototype=Renderer.prototype,NewRenderer}}}),define("taoQtiItem/qtiCommonRenderer/renderers/Renderer",["taoQtiItem/qtiRunner/core/Renderer","taoQtiItem/qtiCommonRenderer/renderers/config"],function(Renderer,config){"use strict";return Renderer.build(config.locations,
config.name,config.options)}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/rubricBlock",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,functionType="function",escapeExpression=this.escapeExpression,self=this;function program2(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'}return(stack1=helpers.unless.call(depth0,depth0&&depth0.empty,{
hash:{},inverse:self.noop,fn:self.program(1,function(depth0,data){var stack1,helper,buffer="";return buffer+='\n<div class="grid-row qti-rubricBlock" data-use="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.use)===functionType?stack1.apply(depth0):stack1)+'" data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="rubricBlock"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='>\n    <div class="col-12">\n        <div class="qti-rubricBlock-body">',((stack1=(helper=helpers.body)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="</div>\n    </div>\n</div>\n"},data),data:data}))||0===stack1?stack1:""})}),define("taoQtiItem/qtiCommonRenderer/renderers/RubricBlock",["tpl!taoQtiItem/qtiCommonRenderer/tpl/rubricBlock",
"taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"rubricBlock",getContainer:containerHelper.get,template:tpl,getData:function(rubric,data){return rubric.isEmpty()&&(data.empty=!0),data}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/stylesheet",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<link \n    href="'+escapeExpression(typeof(stack1=null==(
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.href)===functionType?stack1.apply(depth0):stack1)+'" \n    type="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.type)===functionType?stack1.apply(depth0):stack1)+'" \n    rel="stylesheet" \n    data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" \n    ',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.media,{hash:{},
inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+='media="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.media)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="\n    />"})}),define("taoQtiItem/qtiCommonRenderer/renderers/Stylesheet",["tpl!taoQtiItem/qtiCommonRenderer/tpl/stylesheet","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"stylesheet",template:tpl,getContainer:containerHelper.get}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/table",[
"handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<table\n    class="qti-table',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class
)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='"\n    data-qti-class="table"\n    data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"\n    ',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.summary,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,buffer="";return buffer+='summary="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes
)||!1===stack1?stack1:stack1.summary)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="\n    ",((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="\n    >\n    ",((stack1=(helper=helpers.body)?helper.call(depth0,{
hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="\n</table>\n"})}),define("taoQtiItem/qtiCommonRenderer/renderers/Table",["tpl!taoQtiItem/qtiCommonRenderer/tpl/table","taoQtiItem/qtiCommonRenderer/helpers/container"],function(tpl,containerHelper){"use strict";return{qtiClass:"table",getContainer:containerHelper.get,template:tpl}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/tooltip",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,
Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<span\n    data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"\n    data-qti-class="_tooltip"\n    data-role="tooltip-target"\n    ',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer=""
;return buffer+='lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="\n>\n    ",((stack1=(helper=helpers.body)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="\n</span>\n"})}),define("taoQtiItem/qtiCommonRenderer/renderers/Tooltip",["tpl!taoQtiItem/qtiCommonRenderer/tpl/tooltip","taoQtiItem/qtiCommonRenderer/helpers/container","ui/tooltip"],function(tpl,
containerHelper){"use strict";return{qtiClass:"_tooltip",template:tpl,getContainer:containerHelper.get,render:function(tooltip){containerHelper.get(tooltip).qtip({theme:"default",content:{text:tooltip.content()},position:{target:"event",my:"bottom center",at:"top center"}})}}}),define("taoQtiItem/qtiItem/core/IdentifiedElement",["taoQtiItem/qtiItem/core/Element","taoQtiItem/qtiItem/helper/util"],function(Element,util){"use strict";return Element.extend({buildIdentifier:function(prefix,useSuffix){var item=this.getRootElement(),id=util.buildIdentifier(item,prefix||this.qtiClass,useSuffix);return id&&this.attr("identifier",id),this},id:function(value){
return value||this.attr("identifier")||this.buildIdentifier(this.qtiClass,!0),this.attr("identifier",value)}})}),define("taoQtiItem/qtiItem/core/choices/Choice",["taoQtiItem/qtiItem/core/IdentifiedElement"],function(IdentifiedElement){return IdentifiedElement.extend({init:function(serial,attributes){this._super(serial,attributes)},is:function(qtiClass){return"choice"===qtiClass||this._super(qtiClass)},getInteraction:function(){var found,ret=null,item=this.getRootElement();return item&&(found=item.find(this.serial))&&(ret=found.parent),ret}})}),define("taoQtiItem/qtiItem/core/choices/Hotspot",["taoQtiItem/qtiItem/core/choices/Choice"],function(QtiChoice){
return QtiChoice.extend({})}),define("taoQtiItem/qtiItem/core/choices/AssociableHotspot",["taoQtiItem/qtiItem/core/choices/Hotspot"],function(QtiHotspot){return QtiHotspot.extend({qtiClass:"associableHotspot"})}),define("taoQtiItem/qtiItem/mixin/Mixin",[],function(){return{augment:function(targetClass,methods,options){if("function"==typeof targetClass&&"object"==typeof methods)for(var methodName in methods)if(Object.hasOwnProperty(targetClass.prototype,methodName)){if(options&&options.append){var _parent=targetClass.prototype[methodName];targetClass.prototype[methodName]=function(){return methods[methodName].apply(this,arguments),_parent.apply(this,arguments
)}}}else targetClass.prototype[methodName]=methods[methodName]}}}),define("taoQtiItem/qtiItem/core/Container",["jquery","lodash","taoQtiItem/qtiItem/core/Element","taoQtiItem/qtiItem/helper/rendererConfig"],function($,_,Element,rendererConfig){"use strict";return Element.extend({qtiClass:"_container",init:function(body){if(this._super(),body&&"string"!=typeof body)throw"the body of a container must be a string";this.bdy=body||"",this.elements={}},body:function(body){if(void 0===body)return this.bdy;if("string"!=typeof body)throw"body must be a string";this.bdy=body,$(document).trigger("containerBodyChange",{body:body,container:this,parent:this.parent()})},
setElements:function(elements,body){var returnValue=!1;for(var i in elements){var elt=elements[i];if(!(elt instanceof Element))throw returnValue=!1,"expected a qti element";-1===(body=body||this.bdy).indexOf(elt.placeholder())&&(body+=elt.placeholder()),elt.setRootElement(this.getRootElement()||null),this.elements[elt.getSerial()]=elt,$(document).trigger("containerElementAdded",{element:elt,container:this}),returnValue=!0}return this.body(body),returnValue},setElement:function(element,body){return this.setElements([element],body)},removeElement:function(element){var serial="";return"string"==typeof element?serial=element:element instanceof Element&&(
serial=element.getSerial()),delete this.elements[serial],this.body(this.body().replace("{{"+serial+"}}","")),this},getElements:function(qtiClass){var elts={};if("string"==typeof qtiClass)for(var serial in this.elements)Element.isA(this.elements[serial],qtiClass)&&(elts[serial]=this.elements[serial]);else elts=_.clone(this.elements);return elts},getElement:function(serial){return this.elements[serial]?this.elements[serial]:null},getComposingElements:function(){var elements=this.getElements(),elts={};for(var serial in elements)elts[serial]=elements[serial],elts=_.extend(elts,elements[serial].getComposingElements());return elts},render:function(){
var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),elementsData=[],tpl=this.body();for(var serial in this.elements){var elt=this.elements[serial];if("function"!=typeof elt.render)throw"render() is not defined for the qti element: "+serial;"_container"===elt.qtiClass?tpl=tpl.replace(elt.placeholder(),elt.render(renderer)):(tpl=tpl.replace(elt.placeholder(),"{{{"+serial+"}}}"),elementsData[serial]=elt.render(renderer))}if(renderer.isRenderer)return this._super({body:renderer.renderDirect(tpl,elementsData),contentModel:this.contentModel||"flow"},renderer,args.placeholder)
;throw"invalid qti renderer for qti container"},postRender:function(data,altClassName,renderer){return renderer=renderer||this.getRenderer(),_(this.elements).filter(function(elt){return"function"==typeof elt.postRender}).map(function(elt){return elt.postRender(data,"",renderer)}).flatten(!0).value().concat(this._super(data,altClassName,renderer))},toArray:function(){var arr={serial:this.serial,body:this.bdy,elements:{}};for(var serial in this.elements)arr.elements[serial]=this.elements[serial].toArray();return arr},find:function(serial,parent){var found=null;return this.elements[serial]?found={parent:parent||this,element:this.elements[serial],location:"body"
}:_.each(this.elements,function(elt){if(found=elt.find(serial))return!1}),found},isEmpty:function(){return!this.bdy}})}),define("taoQtiItem/qtiItem/mixin/Container",["taoQtiItem/qtiItem/mixin/Mixin","taoQtiItem/qtiItem/core/Container"],function(Mixin,Container){var methods={initContainer:function(body){this.bdy=new Container(body||""),this.bdy.setRootElement(this.getRootElement()||null),this.bdy.contentModel="blockStatic"},getBody:function(){return this.bdy},body:function(body){var ret=this.bdy.body(body);return body?this:ret},setElement:function(element,body){return this.bdy.setElement(element,body),this},removeElement:function(element){
...
ContainerChoice}),define("taoQtiItem/qtiItem/core/choices/Gap",["taoQtiItem/qtiItem/core/choices/Choice"],function(QtiChoice){return QtiChoice.extend({qtiClass:"gap"})}),define("taoQtiItem/qtiItem/core/Object",["taoQtiItem/qtiItem/core/Element","lodash","taoQtiItem/qtiItem/helper/rendererConfig","taoQtiItem/qtiItem/helper/util"],function(Element,_,rendererConfig,util){return Element.extend({qtiClass:"object",getMediaType:function(){var type="undefined",mimetype=this.attr("type");return mimetype&&(type=0===mimetype.indexOf("video")?"video":0===mimetype.indexOf("audio")?"audio":0===mimetype.indexOf("image")?"image":0===mimetype.indexOf("text/html"
)?"html":"object"),type},render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={};switch(this.getMediaType()){case"video":defaultData.video=!0;break;case"audio":defaultData.audio=!0;break;case"html":defaultData.html=!0;break;case"image":default:defaultData.object=!0}return defaultData.attributes={data:renderer.resolveUrl(this.attr("data"))},defaultData.body=this._alt,this._super(_.merge(defaultData,args.data),args.placeholder,args.subclass,renderer)},isEmpty:function(){return!this.attr("data")}})}),define("taoQtiItem/qtiItem/core/choices/GapImg",[
"taoQtiItem/qtiItem/core/choices/Choice","taoQtiItem/qtiItem/core/Object"],function(QtiChoice,QtiObject){return QtiChoice.extend({qtiClass:"gapImg",initObject:function(object){this.object=object||new QtiObject},getObject:function(){return this.object}})}),define("taoQtiItem/qtiItem/core/choices/GapText",["taoQtiItem/qtiItem/core/choices/ContainerChoice"],function(QtiContainerChoice){"use strict";return QtiContainerChoice.extend({qtiClass:"gapText"})}),define("taoQtiItem/qtiItem/core/choices/HotspotChoice",["taoQtiItem/qtiItem/core/choices/Hotspot"],function(QtiHotspot){return QtiHotspot.extend({qtiClass:"hotspotChoice"})}),define(
"taoQtiItem/qtiItem/mixin/ContainerInline",["taoQtiItem/qtiItem/mixin/Mixin","taoQtiItem/qtiItem/mixin/Container","lodash"],function(Mixin,Container,_){var methods={};return _.extend(methods,Container.methods),_.extend(methods,{initContainer:function(body){Container.methods.initContainer.call(this,body),this.bdy.contentModel="inlineStatic"}}),{augment:function(targetClass){Mixin.augment(targetClass,methods)},methods:methods}}),define("taoQtiItem/qtiItem/core/choices/Hottext",["taoQtiItem/qtiItem/core/choices/Choice","taoQtiItem/qtiItem/mixin/ContainerInline"],function(Choice,Container){var Hottext=Choice.extend({qtiClass:"hottext"});return Container.augment(
Hottext),Hottext}),define("taoQtiItem/qtiItem/core/choices/TextVariableChoice",["jquery","lodash","taoQtiItem/qtiItem/core/choices/Choice","taoQtiItem/qtiItem/helper/rendererConfig"],function($,_,QtiChoice,rendererConfig){return QtiChoice.extend({init:function(serial,attributes,text){this._super(serial,attributes),this.val(text||"")},is:function(qtiClass){return"textVariableChoice"===qtiClass||this._super(qtiClass)},val:function(text){if(void 0===text)return this.text;if("string"!=typeof text)throw"text must be a string";return this.text=text,$(document).trigger("choiceTextChange",{choice:this,text:text}),this},render:function(){
var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={body:this.text};return this._super(_.merge(defaultData,args.data),args.placeholder,args.subclass,renderer)}})}),define("taoQtiItem/qtiItem/core/choices/InlineChoice",["taoQtiItem/qtiItem/core/choices/TextVariableChoice"],function(QtiTextVariableChoice){return QtiTextVariableChoice.extend({qtiClass:"inlineChoice"})}),define("taoQtiItem/qtiItem/core/choices/SimpleAssociableChoice",["taoQtiItem/qtiItem/core/choices/ContainerChoice"],function(QtiContainerChoice){return QtiContainerChoice.extend({qtiClass:"simpleAssociableChoice"})}),define(
"taoQtiItem/qtiItem/core/choices/SimpleChoice",["taoQtiItem/qtiItem/core/choices/ContainerChoice"],function(QtiContainerChoice){return QtiContainerChoice.extend({qtiClass:"simpleChoice"})}),define("taoQtiItem/qtiItem/core/choices/TextEntry",["taoQtiItem/qtiItem/core/choices/TextVariableChoice"],function(QtiTextVariableChoice){return QtiTextVariableChoice.extend({qtiClass:"textEntry"})}),define("taoQtiItem/qtiItem/core/feedbacks/Feedback",["taoQtiItem/qtiItem/core/IdentifiedElement"],function(IdentifiedElement){return IdentifiedElement.extend({is:function(qtiClass){return"feedback"===qtiClass||this._super(qtiClass)}})}),define(
...
ModalFeedback}),define("taoQtiItem/qtiItem/core/Img",["taoQtiItem/qtiItem/core/Element","lodash","taoQtiItem/qtiItem/helper/rendererConfig"],function(Element,_,rendererConfig){return Element.extend({qtiClass:"img",render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),defaultData={};return defaultData.attributes={src:renderer.resolveUrl(this.attr("src"))},this._super(_.merge(defaultData,args.data),args.placeholder,args.subclass,renderer)},isEmpty:function(){return!this.attr("src")}})}),define("taoQtiItem/qtiItem/mixin/NamespacedElement",["taoQtiItem/qtiItem/mixin/Mixin","lodash"],function(Mixin
,_){"use strict";var methods={getNamespace:function(){var relatedItem,namespaces,ns;if(this.ns&&(this.ns.name||this.ns.uri))return _.clone(this.ns);if(relatedItem=this.getRootElement()){for(ns in namespaces=relatedItem.getNamespaces())if(0<namespaces[ns].indexOf(this.nsUriFragment))return{name:ns,uri:namespaces[ns]};return relatedItem.namespaces[this.defaultNsName]=this.defaultNsUri,{name:this.defaultNsName,uri:this.defaultNsUri}}return{}},setNamespace:function(name,uri){this.ns={name:name,uri:uri}}};return{augment:function(targetClass){Mixin.augment(targetClass,methods)},methods:methods}}),define("taoQtiItem/qtiItem/core/Include",[
