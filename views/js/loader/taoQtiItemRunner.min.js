...
"taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/instructions/instructionManager","taoQtiItem/qtiCommonRenderer/helpers/PciResponse","taoQtiItem/qtiCommonRenderer/helpers/patternMask","util/locale","ui/tooltip"],function($,_,__,tpl,containerHelper,instructionMgr,pciResponse,patternMaskHelper,locale,tooltip){"use strict";var deleteTooltip=function($input){$input.data("$tooltip")&&($input.data("$tooltip").dispose(),$input.removeData("$tooltip"))},createTooltip=function($input,theme,message){var textEntryTooltip=tooltip.create($input,message,{theme:theme,trigger:"manual"});$input.data("$tooltip",textEntryTooltip)};return{
qtiClass:"textEntryInteraction",template:tpl,render:function(interaction){var expectedLength,updateMaxCharsTooltip,updatePatternMaskTooltip,attributes=interaction.getAttributes(),$input=interaction.getContainer(),patternMask=interaction.attr("patternMask"),maxChars=parseInt(patternMaskHelper.parsePattern(patternMask,"chars"),10);attributes.expectedLength&&(expectedLength=parseInt(attributes.expectedLength)+2,$input.css("width",expectedLength+"ch"),$input.css("min-width",expectedLength+"ch")),attributes.placeholderText&&$input.attr("placeholder",attributes.placeholderText),maxChars?(updateMaxCharsTooltip=function(){var message,count=$input.val().length
;deleteTooltip($input),message=count?__("%d/%d",count,maxChars):__("%d characters allowed",maxChars),maxChars<=count?($input.addClass("maxed"),createTooltip($input,"warning",message)):($input.removeClass("maxed"),createTooltip($input,"info",message)),$input.data("$tooltip").show()},$input.attr("maxlength",maxChars).on("focus.commonRenderer",function(){_.defer(updateMaxCharsTooltip)}).on("keyup.commonRenderer",function(){_.defer(updateMaxCharsTooltip),containerHelper.triggerResponseChangeEvent(interaction)}).on("blur.commonRenderer",function(){deleteTooltip($input)})):attributes.patternMask?(updatePatternMaskTooltip=function(){var regex=new RegExp(
attributes.patternMask);deleteTooltip($input),$input.val().length&&regex.test($input.val())?$input.removeClass("invalid"):($input.addClass("invalid"),createTooltip($input,"error",__("This is not a valid answer")),$input.data("$tooltip").show())},$input.on("focus.commonRenderer",function(){_.defer(updatePatternMaskTooltip)}).on("keyup.commonRenderer",function(){_.defer(updatePatternMaskTooltip),containerHelper.triggerResponseChangeEvent(interaction)}).on("blur.commonRenderer",function(){deleteTooltip($input)})):$input.on("keyup.commonRenderer",function(){containerHelper.triggerResponseChangeEvent(interaction)})},getContainer:containerHelper.get,
