...
onstart:function(e){var draggableSelector,target,$target=$(e.target);_setActiveGapState($target),$target.addClass("dragged"),draggableSelector=gapFillersSelector,target=e.target,interactUtils.iFrameDragFixOn(function(){activeDrop&&interact(activeDrop).fire({type:"drop",target:activeDrop,relatedTarget:target}),interact(draggableSelector).fire({type:"dragend",target:target})})},onmove:function(e){interactUtils.moveElement(e.target,e.dx,e.dy)},onend:function(e){var $target=$(e.target);_setInactiveGapState($target),$target.removeClass("dragged"),interactUtils.restoreOriginalPosition($target),interactUtils.iFrameDragFixOff()}})).styleCursor(!1)}))};return{
qtiClass:"graphicGapMatchInteraction",template:tpl,render:function(interaction){var self=this;return new Promise(function(resolve){var $container=containerHelper.get(interaction),$gapList=$("ul.source",$container),background=interaction.object.attributes;interaction.gapFillers=[],self.getOption&&self.getOption("enableDragAndDrop")&&self.getOption("enableDragAndDrop").graphicGapMatch&&(isDragAndDropEnabled=self.getOption("enableDragAndDrop").graphicGapMatch),$container.off("resized.qti-widget.resolve").one("resized.qti-widget.resolve",resolve),interaction.paper=graphic.responsivePaper("graphic-paper-"+interaction.serial,interaction.serial,{
width:background.width,height:background.height,img:self.resolveUrl(background.data),imgId:"bg-image-"+interaction.serial,container:$container,resize:function(newSize,factor){$gapList.css("max-width",newSize+"px"),1!==factor&&$gapList.find("img").each(function(){var $img=$(this);$img.width($img.attr("width")*factor),$img.height($img.attr("height")*factor)})}}),_.forEach(interaction.getChoices(),_.partial(_renderChoice,interaction)),_renderGapList(interaction,$gapList),function(interaction){var $container=containerHelper.get(interaction),$gapImages=$("ul > li",$container),bgImage=interaction.paper.getById("bg-image-"+interaction.serial);bgImage&&interact(
...
resetResponse:function(interaction){_shapesUnSelectable(interaction),_.forEach(interaction.gapFillers,function(gapFiller){interactUtils.tapOn(gapFiller.items[2][0])})},destroy:function(interaction){var $container;interaction.paper&&($container=containerHelper.get(interaction),$(window).off("resize.qti-widget."+interaction.serial),$container.off("resize.qti-widget."+interaction.serial),interaction.paper.clear(),instructionMgr.removeInstructions(interaction),$(".main-image-box",$container).empty().removeAttr("style"),$(".image-editor",$container).removeAttr("style"),$("ul",$container).empty(),interact($container.find("ul.source li").selector).unset(),interact(
$container.find(".main-image-box rect").selector).unset()),containerHelper.reset(interaction)},setState:function(interaction,state){_.isObject(state)&&state.response&&(interaction.resetResponse(),interaction.setResponse(state.response))},getState:function(interaction){var state={},response=interaction.getResponse();return response&&(state.response=response),state},isDirectedPairFlipped:isDirectedPairFlipped}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/graphicOrderInteraction",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,
...
;return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n>\n    <option></option>                \n    <option value="empty"></option>\n    ',options={hash:{},inverse:this.noop,fn:this.program(7,program7,data),data:data},stack1=(helper=helpers.choices)?helper.call(depth0,options):typeof(helper=depth0&&depth0.choices)===functionType?helper.call(depth0,options):helper,helpers.choices||(stack1=blockHelperMissing.call(depth0,stack1,{hash:{},inverse:this.noop,fn:this.program(
7,program7,data),data:data})),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n</select>\n"})}),define("taoQtiItem/qtiCommonRenderer/renderers/interactions/InlineChoiceInteraction",["jquery","lodash","i18n","tpl!taoQtiItem/qtiCommonRenderer/tpl/interactions/inlineChoiceInteraction","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/instructions/instructionManager","taoQtiItem/qtiCommonRenderer/helpers/PciResponse","select2","ui/tooltip"],function($,_,__,tpl,containerHelper,instructionMgr,pciResponse){"use strict";var _defaultOptions={allowEmpty:!0,placeholderText:__("select a choice")},_setInstructions=function(
interaction){var required=!!interaction.attr("required"),$container=interaction.getContainer(),$el=$container.select2("container");required&&($el.qtip({theme:"warning",content:{text:__("A choice must be selected")}}),""===$container.val()&&$el.qtip("show"))},_setVal=function(interaction,choiceIdentifier){containerHelper.get(interaction).val(choiceIdentifier).select2("val",choiceIdentifier)};return{qtiClass:"inlineChoiceInteraction",template:tpl,render:function(interaction,options){var opts=_.clone(_defaultOptions),required=!!interaction.attr("required");_.extend(opts,options);var $container=containerHelper.get(interaction)
;opts.allowEmpty&&!required?$container.find("option[value=empty]").text("--- "+__("leave empty")+" ---"):$container.find("option[value=empty]").remove(),$container.select2({width:"element",placeholder:opts.placeholderText,minimumResultsForSearch:-1,dropdownCssClass:"qti-inlineChoiceInteraction-dropdown"});var $el=$container.select2("container");_setInstructions(interaction),$container.on("change",function(e){if($(e.currentTarget).closest(".qti-item").hasClass("prevent-click-handler")){var $selectedIndex=$(e.currentTarget)[0].options.selectedIndex?$(e.currentTarget)[0].options.selectedIndex:null;$container.find("option").one("click",function(e){
e.stopPropagation()}),$container.find("option").eq($selectedIndex).trigger("click")}required&&""!==$container.val()&&$el.qtip("hide"),containerHelper.triggerResponseChangeEvent(interaction)}).on("select2-open",function(){required&&$el.qtip("hide")}).on("select2-close",function(){required&&""===$container.val()&&$el.qtip("show")})},getContainer:containerHelper.get,setResponse:function(interaction,response){_setVal(interaction,pciResponse.unserialize(response,interaction)[0])},getResponse:function(interaction){return pciResponse.serialize(function(interaction){var value=containerHelper.get(interaction).val();return value&&"empty"!==value?[value]:[]}(
...
)===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n  ",((stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:self.noop,fn:self.program(7,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))||0===stack1)&&(buffer+=stack1),
...
;return new Promise(function(resolve){var $container=containerHelper.get(interaction),media=interaction.object,$item=$container.parents(".qti-item"),maxPlays=parseInt(interaction.attr("maxPlays"),10)||0,url=media.attr("data")||"",canBePlayed=function(){var current=parseInt($container.data("timesPlayed"),10);return 0===maxPlays||current<maxPlays},resize=_.debounce(function(){var width,height;interaction.mediaElement&&(height=$container.find(".media-container").height(),width=$container.find(".media-container").width(),interaction.mediaElement.resize(width,height))},200),initMediaPlayer=function(){interaction.mediaElement||(
...
stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:this.noop,fn:this.program(1,function(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+=">\n\t",((stack1=helpers.if.call(depth0,depth0&&depth0.prompt,{hash:{},inverse:this.noop,fn:this.program(3,function(depth0,data){var stack1,helper;return(stack1=(helper=helpers.prompt)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.prompt
)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1?stack1:""},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='\n\t<div class="instruction-container"></div>\n    <div class="file-upload fixed-grid-row lft">\n        <div class="progressbar"></div>\n        <span class="btn-info small col-4"></span>\n        <span class="file-name placeholder col-8 truncate"></span>\n        <input type="file" ',((stack1=helpers.if.call(depth0,depth0&&depth0.accept,{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,helper,buffer="";return buffer+='accept="',stack1=(helper=helpers.accept)?helper.call(depth0,{
hash:{},data:data}):typeof(helper=depth0&&depth0.accept)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'"'},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='/>\n    </div>\n    <div class="file-upload-preview lft visible-file-upload-preview runtime-visible-file-upload-preview">\n        <p class="nopreview">'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"No preview available",options):helperMissing.call(depth0,"__","No preview available",options))
...
buffer+=stack1),buffer+=' class="qti-itemBody ',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class,{hash:{},inverse:this.noop,fn:this.program(5,function(depth0,data){var stack1,buffer="";return buffer+=" "+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1.class)===functionType?stack1.apply(depth0):stack1)},data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+='">',((stack1=(helper=helpers.body)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.body)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(
buffer+=stack1),buffer+='</div>\n    <div id="modalFeedbacks"></div>\n</div>\n'})}),define("taoQtiItem/qtiCommonRenderer/renderers/Item",["tpl!taoQtiItem/qtiCommonRenderer/tpl/item","taoQtiItem/qtiCommonRenderer/helpers/container","taoQtiItem/qtiCommonRenderer/helpers/itemStylesheetHandler"],function(tpl,containerHelper,itemStylesheetHandler){"use strict";return{qtiClass:"assessmentItem",template:tpl,getContainer:containerHelper.get,render:function(item){containerHelper.targetBlank(containerHelper.get(item)),itemStylesheetHandler.attach(item.stylesheets)},destroy:function(item){containerHelper.clear(),item.stylesheets&&itemStylesheetHandler.detach(
item.stylesheets)}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/math",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,functionType="function",escapeExpression=this.escapeExpression,self=this;function program2(depth0,data){var stack1,buffer="";return buffer+=' lang="'+escapeExpression(typeof(stack1=null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"])===functionType?stack1.apply(depth0):stack1)+'"'}return(stack1=helpers.if.call(depth0,depth0&&depth0.block,{hash:{},
inverse:self.program(4,function(depth0,data){var stack1,helper,buffer="";return buffer+='<span data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="math"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))||0===stack1)&&(buffer+=stack1),buffer+="><math>",((stack1=(helper=helpers.raw)?helper.call(depth0,{hash:{},data:data}):typeof(
helper=depth0&&depth0.raw)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="</math></span>"},data),fn:self.program(1,function(depth0,data){var stack1,helper,buffer="";return buffer+='<span data-serial="',stack1=(helper=helpers.serial)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.serial)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" data-qti-class="math"',((stack1=helpers.if.call(depth0,null==(stack1=depth0&&depth0.attributes)||!1===stack1?stack1:stack1["xml:lang"],{hash:{},inverse:self.noop,fn:self.program(2,program2,data),
data:data}))||0===stack1)&&(buffer+=stack1),buffer+='><math display="block">',((stack1=(helper=helpers.raw)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.raw)===functionType?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+="</math></span>"},data),data:data}))||0===stack1?stack1:""})}),define("taoQtiItem/qtiCommonRenderer/renderers/Math",["lodash","core/promise","tpl!taoQtiItem/qtiCommonRenderer/tpl/math","taoQtiItem/qtiCommonRenderer/helpers/container","mathJax"],function(_,Promise,tpl,containerHelper,MathJax){"use strict";return void 0!==MathJax&&MathJax&&(MathJax.Hub.processSectionDelay=0),{
qtiClass:"math",template:tpl,getContainer:containerHelper.get,render:function(math){return new Promise(function(resolve){void 0!==MathJax&&MathJax?(window.MathJax||(window.MathJax=MathJax),_.defer(function(){MathJax.Hub.Queue(["Typeset",MathJax.Hub,containerHelper.get(math).parent()[0]]),MathJax.Hub.Queue(resolve)})):resolve()})}}}),define("tpl!taoQtiItem/qtiCommonRenderer/tpl/modalFeedback",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",
