...
/*    QUnit.module('Register the provider', {
    ...
    QUnit.test('register the qti provider', function(assert){
        ...
        assert.ok(typeof scorer.providers === 'undefined', 'the runner has no providers');

        scorer.register('qti', qtiScoringProvider);

        assert.ok(typeof scorer.providers === 'object', 'the runner has now providers');
        assert.ok(typeof scorer.providers.qti === 'object', 'the runner has now the qti providers');
        assert.equal(scorer.providers.qti, qtiScoringProvider, 'the runner has now the qti providers');
