define([
  ...
  'taoQtiItem/portableLib/OAT/core/promiseQueue',
  'taoQtiItem/portableLib/OAT/core/uuid'
], function (_, promiseQueue, uuid) {
  'use strict';
    ...
    var webStorageFactory = function webStorageFactory(storage) {
      /**
      ...
      var setEntry = function setEntry(storeName, key, value) {
        return new Promise(function (resolve, reject) {
          try {
            storage.setItem("".concat(prefix + storeName, ".").concat(key), JSON.stringify(value));
            resolve(true);
          } catch (ex) {
            reject(ex);
          }
      ...
      var getEntry = function getEntry(storeName, key) {
        return new Promise(function (resolve, reject) {
          ...
          try {
            value = storage.getItem("".concat(prefix + storeName, ".").concat(key));
      ...
      var webStorageBackend = function webStorageBackend(storeName) {
        ...
        return {
          ...
          getItems: function getItems() {
            var keyPattern = new RegExp("^".concat(name));
            return writingQueue.serie(function () {
              return openStore().then(function () {
      ...
      webStorageBackend.getStoreIdentifier = function getStoreIdentifier() {
        var idStore = webStorageBackend(idStoreName); //we use the storeName also as the id
