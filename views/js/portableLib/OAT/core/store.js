define([
  ...
  'taoQtiItem/portableLib/moment.min',
  'module',
  'taoQtiItem/portableLib/OAT/core/logger',
  'taoQtiItem/portableLib/OAT/core/store/localstorage',
  'taoQtiItem/portableLib/OAT/core/store/sessionstorage',
  'taoQtiItem/portableLib/OAT/core/store/indexeddb',
  'taoQtiItem/portableLib/OAT/core/store/memory'
], function (_, moment, module, loggerFactory, localStorageBackend, sessionStorageBackend, indexedDBBackend, memoryBackend) {
  'use strict';
    ...
    sessionStorageBackend = sessionStorageBackend && sessionStorageBackend.hasOwnProperty('default') ? sessionStorageBackend['default'] : sessionStorageBackend;
        ...
        window.navigator.storage.estimate().then(function (estimate) {
          ...
          if (_.isNumber(estimate.usage) && _.isNumber(estimate.quota) && estimate.quota > 0) {
            ...
            if (usedRatio > config.lowSpaceRatio) {
              logger.warn("The browser storage is getting low ".concat(usedRatio.toFixed(2), "% used"), estimate);
              ...
              logger.debug("Browser storage estimate : ".concat(usedRatio.toFixed(2), "% used"), estimate);
            }
          }
        }).catch(function (err) {
          logger.warn("Unable to retrieve quotas : ".concat(err.message));
        });
      }
    ...
    store.cleanUpSpace = function cleanUpSpace(since, storeNamePattern, preselectedBackend) {
      ...
      logger.info("Trying to remove stores lastly opened before ".concat(tsThreshold, "(").concat(since, ")"));
