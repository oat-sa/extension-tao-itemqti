define([
  ...
  'taoQtiItem/portableLib/OAT/core/format',
  'taoQtiItem/portableLib/OAT/core/moduleLoader'
], function (_, format, moduleLoader) {
  'use strict';

    _ = _ && _.hasOwnProperty('default') ? _['default'] : _;
    format = format && format.hasOwnProperty('default') ? format['default'] : format;
    moduleLoader = moduleLoader && moduleLoader.hasOwnProperty('default') ? moduleLoader['default'] : moduleLoader;
    ...
    loggerFactory.load = function load(providerConfigs) {
      ...
      this.providers = []; //we can load the loggers dynamically

      var modules = Object.keys(providerConfigs || {}).map(function (module) {
        return {
          module: module,
          category: 'logger'
        };
      });
      return moduleLoader().addList(modules).load().then(function (loadedProviders) {
        loadedProviders.forEach(function (provider, moduleKey) {
          var providerConfig = modules[moduleKey] && modules[moduleKey].module && providerConfigs[modules[moduleKey].module];

          _this.register(provider, providerConfig);
        });
