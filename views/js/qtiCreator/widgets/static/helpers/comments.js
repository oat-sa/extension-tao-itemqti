define([], function () {
  ...
  const hasAutogeneratedCodeStartComment = (el) => {
    ...
    console.log("inspecting prevNode");
    console.log(previousNode);
    if (!previousNode) {
      return false;
    }
    console.log("trimmed len");
    console.log(previousNode.textContent.trim().length);
    ...
    if (
      ...
      commentFound =
        previousNode.nodeType === 8 &&
        previousNode.textContent &&
        decodeURIComponent(previousNode.textContent).includes(
          autogeneratedCodeStartCommentText
        );
    }
    return commentFound;
  };
  ...
  const hasAutogeneratedCodeEndComment = (el) => {
    ...
    console.log("inspecting nextNode");
    ...
    if (nextNode.textContent && nextNode.textContent.trim().length === 0) {
      ...
      commentFound =
        ...
        decodeURIComponent(nextNode.textContent).includes(
          autogeneratedCodeEndCommentText
        );
    }
    return commentFound;
  };
  const isWrapped = ($el) =>
    hasAutogeneratedCodeStartComment($el[0]) &&
    hasAutogeneratedCodeEndComment($el[0]);
