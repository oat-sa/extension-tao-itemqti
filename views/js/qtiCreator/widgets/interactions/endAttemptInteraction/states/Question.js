define([
    ...
    EndAttemptInteractionStateQuestion.prototype.initForm = function(){
        ...
        var hasRestrictedIdentifier = !!Object.keys(restrictedIdentifiers).length;
    ...
    function _prepareRestrictedIdentifier(widget){
        ...
        if(config && config.responseIdentifiers) {
            for (let identifier in config.responseIdentifiers) {
                if (!config.responseIdentifiers.hasOwnProperty(identifier)) {
                    continue;
                }
                let title = config.responseIdentifiers[identifier];
