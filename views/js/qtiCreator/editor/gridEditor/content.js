define([
    ...
    contentHelper.getContent = function (element, opts) {
        ...
        var $body = options.inner
            ? $(element).clone()
            : $('<div>', { class: 'col-fictive content-helper-wrapper' }).append($(element).clone());
    ...
    contentHelper.destroyGridWidgets = function ($elt, inClone) {
        ...
        $elt.find('[data-writing-mode-class]').each((idx, wmEl) => {
            const $wmEl = $(wmEl);
            $wmEl.addClass($wmEl.attr('data-writing-mode-class')).removeAttr('data-writing-mode-class');
        });
    };
