...
define([
    ...
    /**
     * Set the current item ID - must be called before creating selectors
     * @param {string} itemId - Item identifier
     */
    scaleSelectorFactory.setItemId = function setItemId(itemId) {
        if (!itemId) {
        ...
        if (allScalesPresets.length > 0) {
            syncManager.init(allScalesPresets, currentItemId);
        }
    };
     ...
     */
    scaleSelectorFactory.getItemId = function getItemId() {
    ...
    scaleSelectorFactory.setPresets = function setPresets(presets) {
        if (Array.isArray(presets)) {
            ...
            if (currentItemId) {
                syncManager.init(presets, currentItemId);
            }
        }
    };

    /**
     * Initialize with both presets and item ID
     * @param {Object[]} presets - Array of {uri, label} objects
     * @param {string} itemId - Item identifier
     */
    scaleSelectorFactory.initialize = function initialize(presets, itemId) {
        if (!itemId) {
            throw new Error('itemId is required');
        }
    ...
    /**
     * Reset the factory state - useful when switching between items
     * @param {string} [itemId] - Specific item to reset, or null for all items
     */
    scaleSelectorFactory.reset = function reset(itemId = null) {
        if (itemId) {
            syncManager.reset(itemId);
