test("Load Item", function(){
    ...
    for(var id in allItems){
        ...
        var item = itemLoader.loadItemData(allItems[id].full);
...
test("Load Partial Item", function(){
    
    ok(true, 'getting started');

    for(var id in allItems){
        if(id.indexOf('adaptive') > 0 || id.indexOf('hottext') < 0){
            //adaptive items are not supported yet
        }

        ok(true, '********** Load Variable Content **********');
        var itemLoader = new ItemLoader();
        var item = itemLoader.loadItemData(allItems[id].core);
        itemLoader.loadElements(allItems[id].variable);
    }
...
for(var id in allItems){
    ...
    var itemData = allItems[id].full;

    asyncTest("Render Item " + id, function(){

        var itemLoader = new ItemLoader();
        var item = itemLoader.loadItemData(itemData);
//        CL(itemData, item);
