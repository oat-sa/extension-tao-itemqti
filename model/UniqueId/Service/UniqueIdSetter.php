...
class UniqueIdSetter
{
    ...
    public function __invoke(core_kernel_classes_Resource $item): core_kernel_classes_Resource
    {
        ...
        $originalResourceUri = $this->resourceMetadataRetriever->getOriginalResourceUri($item);

        if ($originalResourceUri === null) {
            $this->logger->info(sprintf('Cannot retrieve original resource URI for item %s', $item->getUri()));

            return $item;
        }

        $originalResource = $item->getResource($originalResourceUri);
        ...
        $this->qtiIdentifierSetter->set($item, $uniqueIdentifier);
