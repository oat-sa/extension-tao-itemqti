 ...
 */
class QtiAuthoringService extends tao_models_classes_GenerisService
{
    ...
    public function createNewItem($itemIdentifier = '', $title = ''){
        ...
        $returnValue = new Item(array(
            'identifier' => $itemIdentifier,
            'title' => empty($title) ? 'QTI item' : $title
        ));

        //add default responseProcessing:
        $returnValue->setResponseProcessing(TemplatesDriven::create($returnValue));
    ...
    public function getData(FlowContainer $element){
        return $this->buildCommonElementTags($element);
    }

    public function getFilteredData(FlowContainer $element){
        $tmpData = $this->getData($element);
        return ItemAuthoring::filterData($tmpData);
    }

    protected function buildCommonElementTags(FlowContainer $element, $bodyData = null){
        ...
        foreach($element->getBody()->getElements() as $element){
            ...
            }else if($element instanceof Math){
    ...
    public function getMathTag(Math $math){
        $display = ($math->attr('display') == 'block') ? 'block' : 'inline';
        return '{{qtiMath:'.$display.':'.$math->getSerial().'}}';
    }

    public function getObjectTag(Object $pObject){
        return '{{qtiObject:'.$pObject->getSerial().'}}';
    }

    public function getInteractionTag(Interaction $interaction){
     ...
     * @return string
     */
    public function getChoiceTag(Choice $choice){

        $returnValue = '';

        if($choice instanceof Hottext){
    ...
    public function getInteractionData(Interaction $interaction){

        $data = '';

        if($interaction instanceof FlowContainer){
            $data = $interaction->getBody()->getBody(); //filterData()??
            if($interaction instanceof GapMatchInteraction){
    ...
    private function getChoices(Element $dataObj, $ordered = true){
        //check type interaction or
        if($dataObj instanceof Interaction || $dataObj instanceof taoQTI_models_classes_QTI_Group){
            
        }
    }

    public function saveItemData(Item $item, $itemBody){
        if(trim($item->getBody()) == trim($itemBody)){
            return true;
        }else{
    ...
    public function saveInteractionData(ContainerInteraction $interaction, $interactionBody){
        return $this->saveData($interaction, $interactionBody);//interaction data must contain blockStatic elements
    }

    public function saveData(FlowContainer $element, $body){
     ...
     */
    public function addInteraction(Item $item, $interactionType, $itemBody){
        ...
        if(class_exists($interactionClass)){
            ...
            if($count){
                ...
                if($interaction instanceof SelectPointInteraction){
                    $response->setHowMatch(Template::MAP_RESPONSE_POINT);
                }
    ...
    public function addObject(Item $item, $itemBody){
     ...
     * 
     * @param oat\taoQtiItem\model\qti\Element
     * @param string
     * @access protected
     * @return string
     */
    protected function restorePlaceholder(Element $qtiElement, $data){

        $pattern = '';
        if($qtiElement instanceof Interaction){
            $pattern = '/{{qtiInteraction:'.$qtiElement->getType().':'.$qtiElement->getSerial().'}}/im';
        }else if($qtiElement instanceof Hottext){
            $pattern = '/{{qtiHottext:'.$qtiElement->getSerial().':([^{]*)}}/im';
        }else if($qtiElement instanceof Gap){
            $pattern = '/{{qtiGap:'.$qtiElement->getSerial().':([^{]*)}}/im';
        }else if($qtiElement instanceof Object){
            $pattern = '/{{qtiObject:'.$qtiElement->getSerial().'}}/im';
        }else if($qtiElement instanceof Math){
    ...
    public function restorePlaceholders(FlowContainer $qtiElement, $data){
    ...
    public function addElement(FlowContainer $elementContainer, $type, $body){
    ...
    public function addMath(Item $item, $itemBody){
    ...
    public function createChoice(Interaction $interaction, $setNumber = null){
        $returnValue = null;
        if(is_null($setNumber)){
            $returnValue = $interaction->createChoice();
        }else if($setNumber === 'gapImg'){
            if($interaction instanceof GraphicGapMatchInteraction){
                $returnValue = $interaction->createGapImg();
            }else{
                throw new QtiModelException('only graphic gap math interaction can have gapImgs');
            }
    ...
    public function createGap(Interaction $interaction, $body){

        $returnValue = null;
        $choice = null;
        $count = 0;
        $newGapPlaceholder = '';
        if($interaction instanceof HottextInteraction){
            $choice = new Hottext();
            $newGapPlaceholder = '{{qtiHottext:new}}';
        }else if($interaction instanceof GapMatchInteraction){
            $choice = new Gap();
    ...
    public function editChoiceData(Choice $choice, $data = ''){
        if(!is_null($choice)){
            $choice->setdata($data);
        }
    }

    public function deleteInteraction(Item $item, $interaction){

        if(!is_string($interaction) && !$interaction instanceof Interaction){
    ...
    public function deleteObject(Item $item, $objectSerial){
        $item->getBody()->removeElement($objectSerial);
    }

    public function deleteChoice(Interaction $interaction, Choice $choice){
    ...
    public function setOptions(Element $qtiObject, $newOptions = array()){
    ...
    public function editOptions(Element $qtiObject, $newOptions = array()){
    ...
    public function setPrompt(BlockInteraction $interaction, $prompt){
    ...
    public function setChoiceContent(Choice $choice, $content){
        if($choice instanceof ContainerChoice){
            $content = $this->restorePlaceholders($choice, $content);
        }
        $choice->setContent($content);
    }

    public function setIdentifier(Element $qtiObject, $identifier){
        ...
        if($qtiObject instanceof Choice || $qtiObject instanceof taoQTI_models_classes_QTI_Group){
            ...
            if(!is_null($item)){
                ...
                throw new QtiModelException('the choice is not associated to any item');
            }
    ...
    public function setResponseProcessing(Item $item, $type, $customRule = ''){
        ...
        if(!is_null($item)){
            ...
            switch(strtolower($type)){
                case 'templatesdriven':{
                        ...
                        try{
                            $responseProcessing = TemplatesDriven::takeOverFrom($item->getResponseProcessing(), $item);
                        }catch(TakeoverFailedException $e){
                            $responseProcessing = TemplatesDriven::create($item);
                ...
                case 'composite':{
                        try{
                            $responseProcessing = Composite::takeOverFrom($item->getResponseProcessing(), $item);
                        }catch(TakeoverFailedException $e){
                            $responseProcessing = Composite::create($item);
    ...
    public function getResponseProcessing(Item $item){
    ...
    public function getInteractionResponse(Interaction $interaction){
        $response = $interaction->getResponse();
    ...
    public function createInteractionResponse(Interaction $interaction){

        $response = new ResponseDeclaration(array(), $interaction->getRelatedItem());
    ...
    public function getInteractionResponseColumnModel(Interaction $interaction, ResponseProcessing $responseProcessing, $isMapping){
        $returnValue = array();
        ...
        switch($interactionType){
            ...
            case 'positionobject':{
                    ...
                    if($rpTemplate == Template::MAP_RESPONSE_POINT){
                        $selectOptions = array(
                            'point' => 'point',
                            'circle' => 'circle',
                            'ellipse' => 'ellipse',
...
//                            'default' => 'default'
    ...
    public function getResponseProcessingType(ResponseProcessing $responseProcessing = null){
        $returnValue = '';

        if($responseProcessing instanceof TemplatesDriven){

            $returnValue = 'templatesdriven';
        }else if($responseProcessing instanceof Custom){

            $returnValue = 'custom';
        }else if($responseProcessing instanceof Template){

            $returnValue = 'customTemplate';
        }else if($responseProcessing instanceof Summation){
     ...
     * 
     * @param oat\taoQtiItem\model\qti\interaction\Interaction $interaction
     * @param string $identifier
     * @return oat\taoQtiItem\model\qti\choice\Choice
     */
    public function getInteractionChoiceByIdentifier(Interaction $interaction, $identifier){

        $eltCollection = $interaction->getIdentifiedElements();
        $returnValue = $eltCollection->getUnique($identifier, 'oat\\taoQtiItem\\model\\qti\\choice\\Choice');

        return $returnValue;
    }

    public function saveInteractionResponse(Interaction $interaction, $responseData){
    ...
    public function updateInteractionResponseOptions(Interaction $interaction){
    ...
    public function getInteractionResponseData(Interaction $interaction){
    ...
    public function setMappingOptions(ResponseDeclaration $response, $mappingOptions = array()){
    ...
    public function isIdentifierUsed(Item $item, $identifier){
        $found = $item->getIdentifiedElement($identifier);
        return !is_null($found);
    }

    public function createFeedbackRule(ResponseDeclaration $response){

        $returnValue = null;

        $relatedItem = $response->getRelatedItem();
        if(!is_null($relatedItem)){
            $outcomeFeedback = new OutcomeDeclaration();
            ...
            $feedbackModal = new ModalFeedback();
            ...
            $returnValue = new SimpleFeedbackRule($outcomeFeedback, $feedbackModal);
            $returnValue->setCondition($response, 'correct');
            $response->addFeedbackRule($returnValue);
        }else{
            throw new QtiModelException('cannot create feedback rule the response is not associated to any item');
        }

        return $returnValue;
    }

    public function deleteFeedbackRule(ResponseDeclaration $response, SimpleFeedbackRule $feedbackRule){
    ...
    public function addFeedbackRuleElse(SimpleFeedbackRule $feedbackRule){
        ...
        if(!is_null($relatedItem)){
            ...
            throw new QtiModelException('cannot create feedback rule else is it is not associated to any item');
        }

        return $returnValue;
    }

    public function deleteFeedbackRuleElse(SimpleFeedbackRule $feedbackRule){
        $returnValue = false;
