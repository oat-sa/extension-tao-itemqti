...
/**
 * Short description of class oat\taoQtiItem\controller\QTIform\AssessmentItem
 *
 * @access public
...
//		$this->form->addElement($labelElt);
		...
		$this->form->addElement(self::createBooleanElement($this->item, 'timeDependent', __('Time dependent')));
//		$this->form->addElement(self::createBooleanElement($this->item, 'adaptive', ''));
     ...
     */
    public static function createBooleanElement( Element $qtiObject, $optionName, $elementLabel = '', $boolean = array('no', 'yes'))
    {
                $boolean = array(__('no'), __('yes'));
		if(count($boolean) != 2){
     ...
     * @return tao_helpers_form_FormElement
     */
    public static function createTextboxElement( Element $qtiObject, $optionName, $elementLabel = '')
    {
