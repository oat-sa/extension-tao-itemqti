...
use oat\taoQtiItem\controller\QtiAuthoring;
...
use oat\taoQtiItem\controller\QTIform\EditObject;
 ...
 */
class QtiAuthoring extends tao_actions_CommonModule
{

    protected $debugMode = false;

    /**
     * @var oat\taoQtiItem\model\qti\Service
     */
    private $qtiService;
    ...
    public function __construct(){
        ...
        $this->service = QtiAuthoringService::singleton();
        $this->defaultData();
    ...
    public function index(){
        ...
        QTISessionCache::singleton()->purge();
        ...
        $this->setData('identifierList', json_encode($identifierList));
    ...
    /**
     * Dump the entire oat\taoQtiItem\model\qti\Item from the session for debugging purpose.
    ...
    protected function cleanPostedData($data, $filterModel = 'blockStatic'){
        ...
        $data = ItemAuthoring::restoreMediaResourceUrl($data);
        $returnValue = ItemAuthoring::cleanHTML($data, $filterModel);
     ...
     * @access protected
     */
    protected function requireChoicesUpdate(Interaction $interaction){
     ...
     * @access protected
     */
    protected function requireInteractionUpdate(Interaction $interaction){
    ...
    public function saveInteraction(){
        ...
        if($myForm->isSubmited()){
            if($myForm->isValid()){
                ...
                if($interaction instanceof BlockInteraction){
                ...
                if($interaction instanceof ObjectInteraction){
                    ...
                    if(isset($values['object_data'])){
                        ...
                        if($interaction instanceof GraphicInteraction){
                        ...
                        }else if($interaction instanceof MediaInteraction){
                ...
                }else if($interaction instanceof ContainerInteraction){
    ...
    public function editResponseProcessing(){
        ...
        $formContainer = new ResponseProcessing($item);
    ...
    public function saveItemResponseProcessing(){
        ...
        echo json_encode(array(
            'saved' => $saved,
            'responseMode' => InteractionAuthoring::isResponseMappingMode($responseProcessingType)
        ));
    }
    ...
    public function saveInteractionResponseProcessing(){
        ...
        if(!is_null($response) && !is_null($rp)){
            if($rp instanceof TemplatesDriven){
                $saved = false;
                ...
                if($this->hasRequestParameter('processingTemplate')){
                    ...
                    if($saved){
                        $setResponseMappingMode = InteractionAuthoring::isResponseMappingMode($processingTemplate);
                    }
                }
            ...
            }elseif($rp instanceof Composite){
                ...
                if($this->hasRequestParameter('interactionResponseProcessing')){
                    ...
                    if($currentClass::CLASS_ID != $classID){
                        $newIRP = InteractionResponseProcessing::create(
    ...
    public function editMappingOptions(){
        ...
        $formContainer = new Mapping($response);
        ...
        if($rp instanceof TemplatesDriven){
            ...
            if($irp instanceof None){
                $form = 'manual';
                        ...
                        'oat\\taoQtiItem\\model\\qti\\response\\interactionResponseProcessing\\MatchCorrectTemplate',
                        'oat\\taoQtiItem\\model\\qti\\response\\interactionResponseProcessing\\MapResponseTemplate',
                        'oat\\taoQtiItem\\model\\qti\\response\\interactionResponseProcessing\\MapResponsePointTemplate'))){
    ...
    public function editResponse(){
        ...
        foreach(InteractionAuthoring::getIRPData($item, $interaction) as $key => $value){
    ...
    public function manageStyleSheets(){
        //create upload form:
        $item = $this->getCurrentItem();
        $formContainer = new CSSuploader($item, $this->getRequestParameter('itemUri'));
        $myForm = $formContainer->getForm();
            ...
            if($myForm->isValid()){
                ...
                if(isset($data['css_import']['uploaded_file'])){
                    ...
                    if(!empty($fileData)){
                        ...
                        $stylesheet = new Stylesheet(array(
        ...
        foreach($item->getStyleSheets() as $css){
            ...
            $cssFiles[] = array(
                'href' => $css->attr('href'),
                'title' => empty($title) ? basename($css->attr('href')) : $title,
    ...
    public function addElement(){
        ...
        if($this->hasRequestParameter('newType') && !is_null($elementContainer)){
            ...
            $filter = 'blockStatic';
            if($elementContainer instanceof Prompt){
                $filter = 'inlineStatic';
            }
    ...
    public function saveData(){
        $filterModel = ($this->getCurrentElement() instanceof Prompt) ? 'inlineStatic' : 'blockStatic';
        ...
        if(is_null($this->currentElement)){
            ...
            if($this->hasRequestParameter('serial') && $this->hasRequestParameter('type')){
                ...
                if($type == 'item'){
                    ...
                    if(is_null($this->currentElement)){
                        throw new QtiModelException('no element found with the serial '.$serial);
                    }
                }
            }
        }
    ...
    public function editObject(){
        ...
        $formContainer = new EditObject($myObject);
        $myForm = $formContainer->getForm();
    ...
    public function __destruct(){
        ...
        $new = $this->qtiService->getItemFromSession($this->item->getSerial());
        /*
        common_Logger::d(print_r($this->item->toArray(), true));
        common_Logger::d(print_r($new->toArray(), true));
        common_Logger::d('********************************__destruct***************************************');
        */
    }
    ...
    protected function renderFeedbackRuleForm(SimpleFeedbackRule $rule){
        ...
        $renderer->setData('conditions', array(
            ...
            'incorrect' => __('response is incorrect'),
